...
const VERSION = 'v206-2026-09-25-dictee-jamais-muette';
...
const STATIC = [
  ...
  './js/config.js',
  // Bundle supabase-js servi depuis notre domaine (fin de la cascade esm.sh,
  // 14/09/2026). Doit etre precache : sans lui, aucune page ne lit de donnees.
  ...
  './js/tel.js',
  './js/heures.js',
  // Importe par chantier.js (trait « aujourd'hui »). Sans lui au precache,
  // le deroulement d'un chantier casse a froid. (15/09/2026)
  './js/jours.js',
  // Correcteur d'orthographe natif, charge par 29 pages. (15/09/2026)
  './js/app-saisie.js',
  ...
  './js/app-erreurs.js',
  './js/app-assistant.js',
  // Prévision du prochain acte (19/09/2026) : chargé par les 13 pages de l'assistant.
  './js/app-prevision.js',
...
self.addEventListener('fetch', (e) => {
  ...
  // ⚠️ VESTIGE — plus aucun code ne passe par ici depuis le 14/09/2026.
  // supabase-js est desormais servi depuis notre domaine
  // (app/js/vendor/supabase-js.js), en UN fichier precache. Ce gestionnaire
  // ne sert plus qu'aux navigateurs qui gardent un vieux HTML en cache le
  // temps que le nouveau SW prenne la main. Ne pas s'en servir pour
  // reintroduire une dependance CDN sur le chemin critique.
  //
  // Historique — LES MODULES DU CDN, cause n°1 des pannes hors ligne (02/09/2026).
  //
  ...
  if (url.origin === location.origin && (/\.(html|js|css|json|woff2)$/i.test(url.pathname) || url.pathname.endsWith('/'))) {
    ...
    // (12/09) `fetch(req)` passait par le cache HTTP du navigateur (max-age 600
    // du CDN) : après une publication, le téléphone gardait l'ancien JS jusqu'à
    // 10 min et l'assistant plantait sur du code déjà corrigé. `no-cache` =
    // revalidation auprès du serveur (ETag → 304 léger), jamais un fichier périmé.
    const reseau = fetch(new Request(req, { cache: 'no-cache' })).then((res) => {
    ...
    // (12/09) 4 s suffisaient a servir un app-actions.js PERIME depuis le cache
    // (300 ko via Cloudflare sur un telephone) : l'assistant tournait sur du code
    // corrige depuis des heures. Le JS/CSS attend le reseau (20 s) ; seul le HTML
    // garde le repli rapide, pour l'ouverture de l'appli.
    const tropLent = new Promise((resolve) => setTimeout(() => resolve(null), isHtml ? 4000 : 20000));
    ...
    e.respondWith((async () => {
      // (12/09) Le commentaire disait « network-first », le code servait le CACHE
      // d'abord : chaque ouverture tournait sur la version précédente (l'assistant
      // « encore cassé » après trois publications). Réseau d'abord, cache en repli.
