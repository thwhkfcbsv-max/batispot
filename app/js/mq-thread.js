...
export async function marquerLuMessagesEquipe(chantierId, monId) {
  try {
    let q = supabase
      ...
      .is('lu_le', null);
    // `.neq()` est du SQL : `auteur_id <> '<id>'` vaut NULL — donc FAUX — quand
    // la colonne est nulle, et la ligne n'est jamais mise a jour. Le comptage,
    // lui, se fait en JS ou `null !== monId` vaut VRAI. Un message sans auteur
    // etait donc compte comme non lu et ne pouvait JAMAIS etre marque lu : la
    // pastille restait a 1 pour toujours. (Constate en prod le 07/09 : une
    // ligne exactement dans ce cas.)
    if (monId) q = q.or(`auteur_id.is.null,auteur_id.neq.${monId}`);
    await q;
      ...
      if (d.dicteeServeurDisponible()) {
        ...
        d.brancherBoutonMaintenu(micBtn, {
          ...
          onFin: (res) => {
            ...
            // (25/09) Le silence se dit, sinon l'artisan croit l'outil cassé.
            if (!texte) {
              dire(res && res.tropCourt
                ? 'Trop court — maintenez le bouton pendant que vous parlez.'
                : ((res && res.message) || "Je n'ai rien entendu, réessayez"));
              return;
            }
