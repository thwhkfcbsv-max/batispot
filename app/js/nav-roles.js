...
//
// ⚠️ Cet appel seul ne suffit pas, et c'est contre-intuitif. Ce fichier est un
// MODULE : il s'exécute avant DOMContentLoaded. Or app-nav.js construit la
// barre SUR DOMContentLoaded. Le filtre partait donc souvent avant l'existence
// des onglets, ne trouvait rien à retirer, et un ouvrier voyait « Devis » et
// « Finances » (mesuré le 15/09 sur metre.html, dont la barre arrive à
// +3090 ms). Sur les écrans légers la course se gagnait par hasard — ce qui
// est pire qu'un défaut franc : deux mesures de suite ne donnaient pas le
// même résultat.
//
// On garde les deux déclenchements. mesDroits() met ses droits en cache : le
// second passage ne refait aucun appel réseau, il ne fait que retirer des
// nœuds. L'opération est idempotente — un onglet déjà retiré ne l'est pas deux
// fois.
filtrerNavigation();
document.addEventListener('bs:nav-rendue', () => { filtrerNavigation(); });
