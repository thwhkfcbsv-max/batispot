...
(function () {
  ...
  // Chantiers = une MAISON (Moctar, 08/09 : correctif 7 du 05/09). L'ancienne
  // icone etait une grille de fenetres d'immeuble : elle se lisait comme un
  // tableau ou un calendrier, pas comme un chantier.
  var ICON_CHANTIERS = '<svg class="bn-ic" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M2.8 10.6 12 3.2l9.2 7.4"/><path d="M5.2 9.1V19.8a1 1 0 0 0 1 1h11.6a1 1 0 0 0 1-1V9.1"/><path d="M9.7 20.8v-6.1h4.6v6.1"/></svg>';
  ...
  // RAFRAÎCHISSEMENT AU CHARGEMENT (07/09/2026, Moctar : « si je reçois un
  // message je veux voir la pastille rouge sur le bandeau en bas à Messages
  // avec le nombre »).
  //
  // Le compte était juste, mais il n'était recalculé QUE depuis dm.html et
  // equipe.html : sur tous les autres écrans la barre affichait la dernière
  // valeur connue, parfois vieille de plusieurs jours. Une pastille qui ment
  // est pire qu'une pastille absente — on cesse de la regarder.
  //
  // Ce fichier est un script classique, sans accès au client Supabase : on
  // passe par le module de messagerie, en dynamique, et sans jamais bloquer
  // l'affichage de la barre si l'appel échoue (hors ligne sur un chantier).
  function rafraichirBadgeAuChargement() {
    if (!document.getElementById('bn-badge-messages')) return;
    import('./mq-thread.js')
      .then(function (m) {
        if (m && typeof m.actualiserBadgeMessages === 'function') return m.actualiserBadgeMessages();
      })
      .catch(function () { /* la dernière valeur connue reste affichée */ });
  }
  ...
  // Au chargement de chaque écran qui porte la barre : on recompte.
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', rafraichirBadgeAuChargement, { once: true });
  } else {
    setTimeout(rafraichirBadgeAuChargement, 0);
  }
  ...
  function init() {
    renderNav();
    renderSousNav();
    // 15/09/2026 — LA COURSE QUI LAISSAIT « Devis » A UN OUVRIER.
    // Le commentaire en tete de ce fichier demande que nav-roles.js s'execute
    // APRES renderNav(). C'est impossible : nav-roles.js est un module, donc
    // differe, donc execute AVANT DOMContentLoaded — l'evenement sur lequel
    // init() est accroche. Le filtre passait sur une barre qui n'existait pas
    // encore et ne retirait rien.
    // Mesure : sur metre.html la barre arrive a +3090 ms (page lourde), le
    // filtre partait bien avant, et l'ouvrier voyait Devis et Finances. Sur
    // les ecrans legers DOMContentLoaded arrive tot et la course se gagnait
    // par hasard — deux mesures de suite donnaient 5 onglets puis 3.
    // On ne compte plus sur l'ordre : la barre annonce qu'elle est posee.
    try {
      document.dispatchEvent(new CustomEvent('bs:nav-rendue'));
    } catch (_e) {
      var ev = document.createEvent('Event');
      ev.initEvent('bs:nav-rendue', true, false);
