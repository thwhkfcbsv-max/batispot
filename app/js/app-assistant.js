...
function bsLibelleLien(href) {
  ...
  function bsInsererCarte(carte) {
    ...
    // (audit 12/09, A45) Sur Finances la nav n'était pas trouvée DANS `mount`
    // (elle vit ailleurs dans le document) : la carte partait après elle, hors
    // de vue. On cherche la nav dans tout le document et on s'insère juste
    // avant, quel que soit son parent.
    ...
    try { carte.style.marginBottom = '12px'; } catch (_) {}
  }
