...
(function() {
  ...
  function bsAstMarquerValidation(logId, valide, corrections) {
    try {
      if (!logId) return;
      const cfg = window.__BATISPOT_CONFIG__ || {};
      ...
      fetch(url, {
        ...
        // (19/09, rang 3) `p_corrections` : les CLÉS que l'artisan a modifiées sur la
        // carte avant de valider — jamais les valeurs (aucun nom, aucun montant).
        body: JSON.stringify(corrections && valide
          ? { p_id: logId, p_valide: true, p_corrections: corrections }
          : { p_id: logId, p_valide: !!valide }),
        keepalive: true,
      }).catch(function () { /* fail-silent */ });
    } catch (_) { /* fail-silent */ }
  }

  // ── LES CARTES ABANDONNEES ────────────────────────────────────────────
  // Mesure du 08/09 sur `assistant_log` : 767 propositions sans verdict, 10
  // validees, ZERO refus. Le code du refus etait pourtant correct — mais
  // l'artisan ne CLIQUE pas « Annuler » : il lit la carte et passe a autre
  // chose. C'est le cas le plus frequent, et c'etait le signal le plus perdu.
  //
  // Une carte laissee sans clic est un refus par inaction : la proposition n'a
  // pas convaincu. On la marque donc `valide = false`, comme un refus
  // explicite — pour la revue, les deux disent la meme chose : cet outil a ete
  // propose a tort. (On les distinguera le jour ou ca servira a decider.)
  const bsAstEnAttente = new Map();          // logId -> true tant que rien n'est clique

  function bsAstSuivre(logId) {
    if (logId) bsAstEnAttente.set(logId, true);
  }
  // Ce que l'artisan a modifié sur la carte : les CLÉS et une mesure de forme
  // (delta de longueur pour les textes). Jamais une valeur — la règle du journal
  // est explicite, aucun nom, aucun montant. `zoneMessage` est le seul champ
  // éditable en place aujourd'hui ; le diff générique couvre le reste.
  function bsAstCorrections(initiaux, finaux, zoneMessage) {
    try {
      const cles = []; const delta = {};
      const a = initiaux || {}; const b = finaux || {};
      Object.keys(Object.assign({}, a, b)).forEach((k) => {
        if (k === 'ensuite' || k === 'resume_humain') return;
        const va = JSON.stringify(a[k] === undefined ? null : a[k]);
        const vb = JSON.stringify(b[k] === undefined ? null : b[k]);
        if (va !== vb) { cles.push(k); if (typeof a[k] === 'string' && typeof b[k] === 'string') delta[k] = b[k].length - a[k].length; }
      });
      if (zoneMessage && typeof zoneMessage.value === 'string') {
        const m0 = typeof a.message === 'string' ? a.message : '';
        const m1 = zoneMessage.value;
        if (m0 !== m1 && !cles.includes('message')) { cles.push('message'); delta.message = m1.length - m0.length; }
      }
      if (!cles.length) return null;
      return { cles: cles.slice(0, 12), nb: cles.length, delta_longueur: delta };
    } catch (_) { return null; }
  }

  function bsAstTrancher(logId, valide, corrections) {
    if (logId) bsAstEnAttente.delete(logId);
    bsAstMarquerValidation(logId, valide, corrections || null);
    // (19/09) Une action validée change l'état : la prévision se relit.
    if (valide && typeof window.bsPrevisionInvalider === 'function') { try { window.bsPrevisionInvalider(); } catch (_) {} }
  }
  /** Texte du modele -> HTML sur. On ECHAPPE d'abord (la sortie du modele
   *  n'est jamais du HTML de confiance : une injection de prompt pourrait
   *  sinon faire executer du script), PUIS on rend le peu de markdown que le
   *  modele produit vraiment. Sans ca l'artisan lisait « **Rythme :** » avec
   *  ses etoiles (capture Moctar, 08/09). L'ordre compte : transformer avant
   *  d'echapper rouvrirait la faille. */
  function bsAstTexteRiche(txt) {
    const safe = String(txt == null ? '' : txt).replace(/[&<>"']/g, (c) => (
      { '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[c]
    ));
    return safe
      .replace(/\*\*([^*\n]+?)\*\*/g, '<strong>$1</strong>')
  ...
  /** Le message qui ouvre la feuille : bonjour, ce qui attend, la question. */
  function bsAstMessageOuverture(ctx, exemples) {
    const h = new Date().getHours();
    const salut = (h >= 18 || h < 5) ? 'Bonsoir' : 'Bonjour';
    let prenom = '';
    try { prenom = typeof window.bsPrevisionPrenom === 'function' ? String(window.bsPrevisionPrenom() || '') : ''; } catch (_) {}
    // (24/09, Moctar : « pas la peine de dire bonjour à chaque fois qu'ils se reconnectent »)
    // On salue UNE fois par jour et par appareil ; ensuite une entrée en matière qui tourne.
    let dejaSalue = false;
    try {
      const cle = 'bs_salut_' + new Date().toISOString().slice(0, 10);
      dejaSalue = localStorage.getItem(cle) === '1';
      if (!dejaSalue) localStorage.setItem(cle, '1');
    } catch (_) {}
    const entrees = ['En quoi puis-je vous aider ?', 'Je vous écoute.', 'Que fait-on ?', 'On avance ?'];
    const entree = entrees[(h + new Date().getMinutes()) % entrees.length];
    const lignes = [dejaSalue ? entree : salut + (prenom ? ' ' + prenom : '') + '.'];
    if (ctx && ctx.chantier_client) lignes.push('Vous êtes sur le chantier de ' + ctx.chantier_client + '.');
    let actes = [];
    try { actes = typeof window.bsPrevisionActes === 'function' ? (window.bsPrevisionActes() || []) : []; } catch (_) {}
    if (actes.length) {
      if (dejaSalue) lignes.length = 0;   // déjà salué : la liste suffit, la question vient après
      lignes.push(actes.length === 1 ? 'Une chose attend :' : 'Ce qui attend :');
      actes.slice(0, 4).forEach((a) => {
        // « Relancer Mme Dupont (devis DEV-2026-0002) » → **Relancer Mme Dupont** — devis DEV-2026-0002
        const m = /^(.*?)\s*\(([^)]*)\)\s*$/.exec(a.acte || '');
        const tete = m ? m[1] : (a.acte || '');
        const detail = [m ? m[2] : '', a.raison || ''].filter(Boolean).join(', ');
        lignes.push('• **' + tete + '**' + (detail ? ' — ' + detail : ''));
      });
      if (actes.length > 4) lignes.push('… et ' + (actes.length - 4) + ' autre' + (actes.length - 4 > 1 ? 's' : '') + '.');
      lignes.push('', actes.length === 1 ? 'On s\'en occupe ? Dites-le-moi, ou touchez la proposition.' : 'Par lequel on commence ? Dites-le-moi, ou touchez une proposition.');
    } else {
      lignes.push('', 'Rien n\'attend aujourd\'hui. Que voulez-vous faire ?' + (exemples ? ' Par exemple : ' + exemples + '.' : ''));
    }
    return lignes.join('\n');
  }

  /** Toute carte encore en attente est abandonnee : on le consigne. */
  function bsAstSolderAbandons() {
    if (!bsAstEnAttente.size) return;
    const ids = Array.from(bsAstEnAttente.keys());
    bsAstEnAttente.clear();
    ids.forEach(function (id) { bsAstMarquerValidation(id, false); });
  }
  // Quand l'artisan pose une AUTRE question, il a tourne la page : ce qui
  // restait a valider ne le sera plus.
  window.bsAstSolderAbandons = bsAstSolderAbandons;
  // `pagehide` et non `beforeunload` : le seul qui parte de facon fiable sur
  // iOS quand l'appli passe en arriere-plan. `keepalive` est deja pose sur la
  // requete, elle survit a la fermeture.
  window.addEventListener('pagehide', bsAstSolderAbandons);
  document.addEventListener('visibilitychange', function () {
    if (document.visibilityState === 'hidden') bsAstSolderAbandons();
  });
  ...
  if (!document.getElementById(styleId)) {
    ...
    style.textContent = `
      ...
      .bs-ast-msg {
        ...
        font-size: 13.5px;
        line-height: 1.5;
        /* Une chaîne sans espace — un lien de suivi, un numéro de devis, une
           adresse e-mail — étirait la bulle au-delà du panneau et faisait
           défiler l'écran de côté sur un téléphone. */
      ...
      .bs-ast-input {
        ...
        background: #FFFFFF !important;
        color: #1C2B22 !important;
        /* (24/09, Moctar) « l'encadré devrait être petit et s'agrandir en fonction de
           ce qu'on écrit » : une ligne au repos, il suit le texte jusqu'à ~5 lignes,
           puis défile. Un contenteditable grandit seul ; on borne. */
        line-height: 1.35;
        min-height: 40px;
        max-height: 128px;
        overflow-y: auto;
...
`;
  ...
  function initAssistantUI() {
    ...
    overlay.innerHTML = `
      <div class="bs-ast-sheet" id="bsAstSheet">
        ...
        <div class="bs-ast-input-bar">
          ...
          <div contenteditable="true" role="textbox" aria-multiline="false" aria-label="Posez une question ou dictez" enterkeyhint="send" autocapitalize="sentences" autocomplete="off" spellcheck="true" class="bs-ast-input" id="bsAstInput" data-placeholder="Écrivez ou dictez…"></div><!-- (12/09, Moctar : « il faut absolument résoudre la clé, la carte en haut du clavier ») Chrome Android affiche sa barre de remplissage (adresse / clé / carte) sur TOUT <input> ou <textarea>, autocomplete=off ou non (essayé le 06/09, sans effet). Il ne l'affiche pas sur un contenteditable : c'en est un, avec un shim .value (voir bsAstShimSaisie) pour que le reste du code ne change pas. -->
    ...
    // Shim : le champ est un contenteditable (barre de remplissage Chrome), mais
    // tout le module lui parle comme à un <input> (.value, .disabled, .placeholder…).
    (function bsAstShimSaisie(el) {
      if (!el || el.tagName === 'INPUT' || el.tagName === 'TEXTAREA' || el.__bsShim) return;
      el.__bsShim = true;
      Object.defineProperty(el, 'value', {
        get() { return (el.innerText || '').replace(/\u00a0/g, ' ').replace(/\n+$/, ''); },
        set(v) { el.textContent = v == null ? '' : String(v); if (!el.textContent) el.innerHTML = ''; },
      });
      Object.defineProperty(el, 'disabled', {
        get() { return el.getAttribute('contenteditable') === 'false'; },
        set(v) { el.setAttribute('contenteditable', v ? 'false' : 'true'); el.setAttribute('aria-disabled', v ? 'true' : 'false'); },
      });
      Object.defineProperty(el, 'placeholder', {
        get() { return el.getAttribute('data-placeholder') || ''; },
        set(v) { el.setAttribute('data-placeholder', v == null ? '' : String(v)); },
      });
      Object.defineProperty(el, 'type', { get() { return 'text'; } });
      el.select = function () { try { const r = document.createRange(); r.selectNodeContents(el); const sel = getSelection(); sel.removeAllRanges(); sel.addRange(r); } catch (_) {} };
      el.setSelectionRange = function () { try { const r = document.createRange(); r.selectNodeContents(el); r.collapse(false); const sel = getSelection(); sel.removeAllRanges(); sel.addRange(r); } catch (_) {} };
      // Un seul paragraphe : Entrée = envoyer (le keydown du module fait le
      // reste) ; un collage arrive en texte brut, jamais en HTML.
      ...
      st.textContent = '.bs-ast-input[contenteditable]{min-height:44px;max-height:120px;overflow-y:auto;white-space:pre-wrap;overflow-wrap:anywhere;line-height:1.35;padding-top:11px;padding-bottom:11px;cursor:text;-webkit-user-modify:read-write-plaintext-only}'
        + '.bs-ast-input[contenteditable]:empty::before{content:attr(data-placeholder);color:#8A9B93;pointer-events:none}'
        + '.bs-ast-input[contenteditable="false"]{opacity:.6}';
    ...
    fab.addEventListener('click', () => {
      ...
      if (!document.body.classList.contains('bs-demo-en-cours')) setTimeout(() => input.focus(), 150);   // (12/09) pas de clavier pendant la démo
    });
    ...
    // (12/09, Moctar : « l'assistant doit reconnaître tout ça via une photo et
    // l'intégrer au bon endroit ») : l'appareil photo demande d'abord CE QUE
    // c'est — une pièce à mesurer, un ticket ou une facture fournisseur, un de
    // ses devis. Chaque type suit son propre chemin, sans quitter l'assistant.
    camBtn.addEventListener('click', () => bsProposerTypePhoto());
    function bsProposerTypePhoto() {
      const m = document.createElement('div');
      m.className = 'bs-ast-msg bot';
      const t = document.createElement('div');
      t.textContent = 'Que voulez-vous photographier ?';
      t.style.marginBottom = '8px';
      const rangee = document.createElement('div');
      rangee.style.cssText = 'display:flex;flex-direction:column;gap:6px;';
      const choix = [
        ['Une pièce à mesurer (métré)', 'metre'],
        ['Un ticket ou une facture fournisseur', 'ocr'],
        ['Un de mes devis (pour ma grille de prix)', 'devis'],
      ];
      for (const [libelle, action] of choix) {
        const b = document.createElement('button');
        b.type = 'button'; b.className = 'bs-ast-card-btn'; b.textContent = libelle;
        b.style.cssText = 'text-align:left;';
        b.addEventListener('click', () => { m.remove(); triggerCamera(action); });
    ...
    hiddenFileInput.addEventListener('change', (e) => {
      ...
      // Le NOM DU FICHIER est une entree utilisateur : il partait tel quel
      // dans innerHTML (audit du 08/09). Une photo nommee avec des balises
      // s'executait dans la page. On construit noeud par noeud : textContent
      // ne peut rien executer, et l'URL passe par un attribut, pas par du HTML.
      uMsg.replaceChildren();
      const vign = document.createElement('img');
      vign.src = imgUrl;
      vign.style.cssText = 'width:100%;max-width:200px;border-radius:8px;display:block;margin-bottom:6px;';
      const leg = document.createElement('span');
      leg.textContent = `Photo analysée (${file.name})`;
      uMsg.append(vign, leg);
      messages.appendChild(uMsg);
      messages.scrollTop = messages.scrollHeight;

      const bMsg = document.createElement('div');
      bMsg.className = 'bs-ast-msg bot';
      bMsg.textContent = act === 'ocr' ? "Lecture du ticket en cours…"
        : act === 'devis' ? "Lecture de votre devis en cours…" : "Analyse de la photo en cours…";
      ...
      (async () => {
        ...
        try {
          if (act === 'ocr') {
            ...
            bsProposerImportPrix(bMsg, file);
          } else if (act === 'devis') {
            bMsg.textContent = '';
            const intro = document.createElement('div');
            intro.textContent = 'Les prix lus sur votre devis : cochez ceux à garder dans votre grille.';
            bMsg.appendChild(intro);
            await bsMonterImportPrix(bMsg, file, 'vente');
          } else {
...
// ── Photo → prix d'achat / grille de vente, sans quitter l'assistant (12/09) ──
// Réutilise le module de Mon entreprise (app-import-devis.js) : même lecture,
// mêmes cases à cocher, même enregistrement. Ici on le monte dans la bulle et
// on lui donne la photo directement.
const bsScriptsCharges = {};
function bsChargerScript(src) {
  if (!bsScriptsCharges[src]) {
    bsScriptsCharges[src] = new Promise((resolve, reject) => {
      const sc = document.createElement('script');
      sc.src = src; sc.onload = resolve; sc.onerror = () => reject(new Error('script ' + src));
      document.head.appendChild(sc);
    });
  }
  return bsScriptsCharges[src];
}
async function bsMonterImportPrix(bMsg, file, cible) {
  try {
    if (cible === 'vente' && !window.BtpPriceGridManager) await bsChargerScript('./js/app-knowledge-btp.js?v=2026_09_05');
    if (!window.BsImportDevis) await bsChargerScript('./js/app-import-devis.js?v=2026_09_06e');
  } catch (e) {
    const err = document.createElement('div');
    err.style.cssText = 'font-size:12px;color:#B45309;margin-top:6px;';
    err.textContent = "Le module d'import n'a pas pu se charger. Réessayez depuis Mon entreprise › Grille de prix.";
    bMsg.appendChild(err); return;
  }
  const cont = document.createElement('div');
  cont.style.cssText = 'margin-top:8px;';
  bMsg.appendChild(cont);
  window.BsImportDevis.monter(cont, { cible, onImport: () => {
    const ok = document.createElement('div');
    ok.style.cssText = 'font-size:12.5px;color:var(--t1);font-weight:700;margin-top:6px;';
    ok.textContent = cible === 'achat' ? "Prix d'achat enregistrés. Ils servent au calcul de vos marges." : 'Prix ajoutés à votre grille. Vos prochains devis les proposent.';
    bMsg.appendChild(ok);
  } });
  // Le module affiche son titre, son texte et son bouton « Choisir » : ici la photo est déjà là.
  cont.querySelectorAll('strong, p, [data-bs="choisir"]').forEach((el) => { if (el.closest('[data-bs="resultats"]') === null) el.style.display = 'none'; });
  const champ = cont.querySelector('[data-bs="fichier"]');
  if (!champ) return;
  try {
    const dt = new DataTransfer(); dt.items.add(file); champ.files = dt.files;
    champ.dispatchEvent(new Event('change', { bubbles: true }));
  } catch (e) {
    const err = document.createElement('div');
    err.style.cssText = 'font-size:12px;color:#B45309;margin-top:6px;';
    err.textContent = 'Impossible de transmettre la photo au module. Réessayez depuis Mon entreprise › Grille de prix.';
    bMsg.appendChild(err);
  }
  bMsg.parentElement && (bMsg.parentElement.scrollTop = bMsg.parentElement.scrollHeight);
}
function bsProposerImportPrix(bMsg, file) {
  const bloc = document.createElement('div');
  bloc.style.cssText = 'margin-top:10px;display:flex;flex-direction:column;gap:6px;';
  const t = document.createElement('div');
  t.style.cssText = 'font-size:12.5px;color:var(--t1);font-weight:700;';
  t.textContent = 'Cette photo contient aussi des prix ?';
  const b1 = document.createElement('button');
  b1.type = 'button'; b1.className = 'bs-ast-card-btn'; b1.style.cssText = 'text-align:left;';
  b1.textContent = "Facture fournisseur → enregistrer mes prix d'achat";
  const b2 = document.createElement('button');
  b2.type = 'button'; b2.className = 'bs-ast-card-btn'; b2.style.cssText = 'text-align:left;';
  b2.textContent = 'Un de mes devis → ajouter les prix à ma grille';
  b1.addEventListener('click', () => { bloc.remove(); bsMonterImportPrix(bMsg, file, 'achat'); });
  b2.addEventListener('click', () => { bloc.remove(); bsMonterImportPrix(bMsg, file, 'vente'); });
  bloc.append(t, b1, b2);
  bMsg.appendChild(bloc);
}
...
function bsAfficherMetre(bMsg, d) {
  ...
  avert.style.cssText = 'font-size:12.5px;color:#B45309;margin-top:6px;';
...
function bsLibelleLien(href) {
  const h = String(href || '');
  // L'onglet se teste AVANT la page : './finances.html?onglet=factures' contient
  // « finances », donc sans ce test le bouton annoncait « Voir mes finances »
  // alors qu'il ouvre les factures (retour terrain Moctar, 08/09/2026).
  if (h.indexOf('onglet=factures') >= 0) return 'Voir mes factures →';
  if (h.indexOf('onglet=depenses') >= 0) return 'Voir mes dépenses →';
  ...
  return 'Ouvrir →';
    ...
    // Combien d'allers-retours l'assistant a le droit de faire avant de rendre
    // la main. C'était 2 : il pouvait chercher le chantier, lister les devis,
    // puis il DEVAIT s'arrêter et parler — sans jamais atteindre l'action
    // demandée. Mesuré au banc le 08/09 : « met 2 prises au lieu de 1 sur le
    // chiffrage de Martin » donnait chercher_chantier → lister_devis → rien.
    //
    // On monte à 5. Ce qui borne la boucle n'est PAS ce chiffre :
    //   — toute ÉCRITURE passe par une carte que l'artisan valide à la main
    //     (bsAstRendreProposition), donc rien ne part sans lui ;
    //   — seules les LECTURES s'enchaînent toutes seules, et elles ne
    //     modifient rien ;
    //   — chaque tour coûte un appel : à 5, le pire cas reste sous les 10 s.
    // Le chiffre est ici, nommé, pour qu'on puisse le remonter ou le baisser
    // sur une mesure et non sur une impression.
    const BS_TOURS_MAX = 5;

    async function bsAstSynthetiser(question, nom, params, res, messages, profondeur, signature, bMsg, carte) {
      if (!question || profondeur >= BS_TOURS_MAX) return;
      ...
      try {
        ...
        const hist = bsAstHistorique.map((m) => ({ role: m.role, content: m.content }));
        ...
        const r = await fetch(url, { method: 'POST', headers, signal: ctrl.signal, body: JSON.stringify({
          ...
          contexte_ecran: bsAstContexteEcran(), ...bsAstPrevuCorps(),
        ...
        const rendu = bsAstTexteRiche(txt);
        let cible;
        if (enPlace) {
          attente.remove();
          intro.remove();
          ...
          bulle.innerHTML = rendu;
          cible = bulle;
        }
        messages.scrollTop = messages.scrollHeight;
        // PLUSIEURS actions (08/09) : l'artisan qui vide sa journee en une
        // phrase obtient une carte PAR action, dans l'ordre demande. Le
        // serveur renvoie `actionProposals` ; `actionProposal` au singulier
        // reste la premiere, pour un client pas encore a jour.
        const props = (j && Array.isArray(j.actionProposals) && j.actionProposals.length)
          ? j.actionProposals
          : (j && j.actionProposal ? [j.actionProposal] : []);
        props.forEach((pr) => bsAstRendreProposition(pr, cible, messages, j.log_id, question, profondeur + 1));
      } catch (_) {
        repli();
      }
    }
    ...
    async function bsAstRendreProposition(proposition, bMsg, messages, logId, question, profondeur) {
      ...
      // (19/09, rang 3 de l'étude) Photo des paramètres tels que proposés : au clic
      // « Valider », on compare pour savoir ce que l'artisan a CORRIGÉ. Cinq fiches
      // sur douze réclament ce signal ; on ne gardait qu'un booléen.
      const bsAstParamsInitiaux = JSON.parse(JSON.stringify(proposition.params || {}));
      const carte = document.createElement('div');
      carte.className = 'bs-ast-card-result';
      ...
      try {
        ...
        // Cible ambiguë (« Pose » quand cinq étapes en portent le nom) : on
        // pose la question au lieu d'afficher un « Valider » qui ne peut
        // qu'échouer. (Moctar, 08/09.)
        if (typeof mod.questionAvantAction === 'function') {
          let q = null;
          try { q = await mod.questionAvantAction(nom, proposition.params); } catch (_) { q = null; }
          if (q) {
            const bloc = document.createElement('div');
            bloc.style.cssText = 'font-size:12.5px;color:var(--t1);margin-top:6px;';
            bloc.textContent = q;
      ...
      if (detail) {
        ...
        precision.style.cssText = 'font-size:12.5px;color:var(--t1);margin-bottom:6px;';
        precision.textContent = detail;
        carte.appendChild(precision);
      }
      // Même chose quand il faut lire la base pour le dire (export comptable :
      // période en toutes lettres, e-mail du comptable, ou son absence).
      if (typeof mod.detailPropositionAsync === 'function') {
        try {
          const d2 = await mod.detailPropositionAsync(nom, proposition.params);
          if (d2) {
            const precision2 = document.createElement('div');
            precision2.style.cssText = 'font-size:12.5px;color:var(--t1);margin-bottom:6px;';
            precision2.textContent = d2;
            carte.appendChild(precision2);
          }
        } catch (_) {}
      }
      ...
      if (typeof mod.apercuMessage === 'function') {
        try {
          ...
          if (ap && ap.texte) {
            ...
            lab.style.cssText = 'font-size:12.5px;color:#5A7268;margin:2px 0 4px;';
            ...
            // (Moctar, 12/09 : « et correcteur d'orthographe ») — le même « Aa »
            // que la messagerie (mq-thread.js), sur le message que l'artisan
            // relit avant de valider : ce qui part au client ou à l'équipe est propre.
            try {
              const corr = document.createElement('button');
              corr.type = 'button';
              corr.className = 'bs-ast-chip';
              corr.textContent = "Aa Corriger l'orthographe";
              corr.style.cssText = 'margin:2px 0 8px;font-size:12px;height:30px;';
              corr.addEventListener('click', async () => {
                const brut = zoneMessage.value.trim();
                if (!brut || corr.disabled) return;
                corr.disabled = true; const avant = corr.textContent; corr.textContent = 'Correction…';
                try {
                  ...
                  if (res && res.texte && res.texte.trim() && res.texte.trim() !== brut) {
                    zoneMessage.value = res.texte.trim();
                    corr.textContent = res.nb_corrections ? `✓ ${res.nb_corrections} correction${res.nb_corrections > 1 ? 's' : ''} — relisez` : '✓ Corrigé — relisez';
                  } else {
                    corr.textContent = '✓ Rien à corriger';
                  }
                } catch (_) { corr.textContent = 'Correction indisponible'; }
                finally { setTimeout(() => { corr.textContent = avant; corr.disabled = false; }, 2500); }
              });
              carte.appendChild(corr);
            } catch (_) {}
          }
      ...
      rappel.style.cssText = 'font-size:12.5px;color:#5A7268;margin-bottom:8px;';
      ...
      // Une carte a valider existe : on la suit jusqu'a ce qu'elle soit
      // tranchee, ou soldee comme abandonnee.
      bsAstSuivre(logId);

      const valider = document.createElement('button');
      valider.className = 'bs-ast-card-btn bs-ast-valider';
      valider.type = 'button';
      // (12/09, Moctar : « le dernier message de Benali pour la relance ne figure
      // pas dans les messages ») — il avait validé un BROUILLON ; l'envoi
      // demandait un second bouton. Pour une relance, valider = envoyer.
      valider.textContent = destructif ? LIBELLE_SUPPRIMER : (nom === 'preparer_relance' ? 'Envoyer au client' : 'Valider');
      ...
      async function bsAstExecuter() {
        ...
        bsAstTrancher(logId, true, bsAstCorrections(bsAstParamsInitiaux, proposition.params, zoneMessage));
        ...
        try {
          ...
          const res = await mod.executerAction(nom, proposition.params);
          valider.remove(); annuler.remove();
          // (Moctar, 09/09 : « est-ce que le message est vraiment envoyé ? »)
          // Une fois l'action faite, la carte ne doit plus dire « rien n'est
          // enregistré » ni « validez ci-dessous » : on retire le rappel et
          // l'intro générique du serveur, il ne reste que le résultat.
          try { rappel.remove(); } catch (_) {}
          try {
            Array.from(bMsg.childNodes).forEach((n) => {
              if (n === carte || (n.nodeType === 1 && n.classList && n.classList.contains('bs-ast-card-result'))) return;
              const t = (n.textContent || '').trim();
              if (/^Je vous propose l'action suivante/i.test(t) || /^Je vous pr[ée]pare /i.test(t)) n.remove();
            });
          } catch (_) {}
          etat.style.color = 'var(--t1)';
          etat.textContent = '✓ ' + res.message;
          // (Moctar, 12/09 00h50 : « l'assistant devrait me le confirmer ») —
          // une vraie BULLE de l'assistant, pas seulement une ligne dans la
          // carte, et elle entre dans le fil : la conversation sait que c'est fait.
          try {
            const conf = document.createElement('div');
            conf.className = 'bs-ast-msg bot';
            const heure = new Date().toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' }).replace(':', 'h');
            conf.textContent = `✓ C'est fait (${heure}) : ${res.message}`;
            messages.appendChild(conf);
            bsAstHistPush('model', `Fait à ${heure} : ${res.message}`);
            messages.scrollTop = messages.scrollHeight;
          } catch (_) {}
          ...
          if (res.geste && res.geste.envoyer_client && nom === 'preparer_relance') {
            // Valider = envoyer (voir le libellé du bouton) : on envoie tout de
            // suite, et le bouton ci-dessous ne sert qu'en cas d'échec.
            const g0 = res.geste.envoyer_client;
            try {
              const r0 = await mod.envoyerAuClient({ chantier_id: g0.chantier_id, texte: g0.texte });
              etat.textContent = '✓ ' + ((r0 && r0.message) || 'Message envoyé au client.') + (g0.client_name ? ' (' + g0.client_name + ')' : '');
              try { document.dispatchEvent(new CustomEvent('bs:action-executee', { detail: { action: 'envoyer_au_client', params: g0, res: r0 } })); } catch (_) {}
              res.geste = null;
            } catch (e0) {
              etat.style.color = '#B91C1C';
              etat.textContent = (e0 && e0.message) || "L'envoi a échoué. Rien n'est parti — réessayez avec le bouton.";
            }
          }
    ...
    // UN SEUL FIL qui survit aux écrans (12/09, Moctar : « est-ce que le chat
    // garde le fil ou repart de zéro ? »). Il repartait de zéro à chaque page —
    // y compris quand l'assistant lui-même ouvrait un écran. Le fil est gardé
    // sur le téléphone (par compte) et repris tant que le dernier échange a
    // moins de 2 h ; au-delà, on repart propre. « Nouvelle conversation » coupe.
    const BS_AST_FIL_TTL_MS = 2 * 3600 * 1000;
    function bsAstFilCle() {
      // L'identifiant est le `sub` du jeton de session : stable d'un jeton à
      // l'autre (le jeton, lui, change toutes les heures).
      let id = '';
      try {
        const j = bsJetonSession() || '';
        const p = j.split('.')[1] || '';
        id = (JSON.parse(atob(p.replace(/-/g, '+').replace(/_/g, '/'))) || {}).sub || '';
      } catch (_) {}
      return 'bs_ast_fil:' + (id || 'anon');
    }
    function bsAstFilCharger() {
      try {
        const brut = JSON.parse(localStorage.getItem(bsAstFilCle()) || 'null');
        if (!brut || !Array.isArray(brut.messages) || !brut.t) return [];
        if (Date.now() - Number(brut.t) > BS_AST_FIL_TTL_MS) return [];
        return brut.messages.filter((m) => m && (m.role === 'user' || m.role === 'model') && typeof m.content === 'string').slice(-12);
      } catch (_) { return []; }
    }
    function bsAstFilSauver() {
      try { localStorage.setItem(bsAstFilCle(), JSON.stringify({ t: Date.now(), messages: bsAstHistorique })); } catch (_) {}
    }
    window.bsAstNouvelleConversation = function () {
      bsAstHistorique = [];
      try { localStorage.removeItem(bsAstFilCle()); } catch (_) {}
      const zone = document.getElementById('bsAstMessages');
      if (zone) zone.replaceChildren();
      chipsContexteIntro = false;
      if (typeof bsAstPucesContextuelles === 'function') bsAstPucesContextuelles();
    };
    let bsAstHistorique = bsAstFilCharger();
    function bsAstHistPush(role, texte) {
      const t = String(texte == null ? '' : texte).trim();
      if (!t) return;
      // (12/09, retour de Gabriel) `content` (2 000 caractères) est ce qu'on renvoie au
      // modèle ; `affichage` garde le texte entier pour le réaffichage à la réouverture —
      // une réponse longue se coupait sur « 8. ** » après fermeture/réouverture.
      bsAstHistorique.push({ role: role === 'user' ? 'user' : 'model', content: t.slice(0, 2000), affichage: t.length > 2000 ? t.slice(0, 12000) : undefined });
    ...
    async function bsContexteDonnees() {
      ...
      try {
        ...
        // (12/09) La BASE fait autorité : la copie locale du téléphone ne contient
        // que les devis créés ICI — vide sur un téléphone neuf, le serveur écrivait
        // « Devis : 0 au total » et l'assistant répondait « vous n'avez aucun devis
        // enregistré » à un artisan qui en a 13 (Moctar, 23h06 et 02h26).
        let enBase = [];
        try {
          enBase = (await store.listerDevis()).slice(0, 20).map((d) => ({
            numero: d.numero || d.id, client: (d.chantiers && d.chantiers.client_name) || d.client_name || '',
            ttc: Number(d.total_ttc) || 0, statut: d.status || 'brouillon',
          }));
        } catch (_) {}
        const numerosBase = new Set(enBase.map((d) => d.numero));
        const locauxNonPartis = store.lireLocal().filter((e) => e.sync !== 'ok').slice(0, 10).map((e) => {
        ...
        }).filter((d) => !numerosBase.has(d.numero));
        out.devis = enBase.concat(locauxNonPartis).slice(0, 20);
      } catch (_) {}
      try {
        const sb = await import('./supabase.js');
        ...
        // L'ÉQUIPE (12/09, Moctar : seul → l'assistant ne propose jamais de
        // « prévenir l'équipe »). Membres actifs autres que l'artisan lui-même.
        try {
          let moi = '';
          try { const j = bsJetonSession() || ''; const pl = j.split('.')[1] || ''; moi = (JSON.parse(atob(pl.replace(/-/g, '+').replace(/_/g, '/'))) || {}).sub || ''; } catch (_) {}
          const membres = (await sb.listMembres()) || [];
          const actifs = membres.filter((m) => m && m.statut === 'actif' && m.user_id !== moi);
          out.equipe = { nombre: actifs.length, prenoms: actifs.map((m) => m.prenom).filter(Boolean).slice(0, 8) };
        } catch (_) {}
        // (A58, audit 12/09) QUI PARLE : patron, chef ou compagnon — le serveur
        // en tient compte (un compagnon ne voit pas les finances, ne supprime
        // rien, transmet au patron).
        try {
          const d = await sb.mesDroits();
          if (d && d.role) out.role = { nom: String(d.role), patron: d.patron === true, encadrement: d.encadrement === true, voit_marges: d.voit_marges === true };
        } catch (_) {}
        // Étapes à faire ou en cours des chantiers en cours (12/09) : « on a
        // bouclé la pose du meuble vasque » → le modèle lit ici de quel
        // chantier il s'agit au lieu de le demander (banc cov-02, couverture
        // 11/09 ×3). Une requête, 40 lignes max, jamais les terminées.
        try {
          const parId = {};
          (chs || []).forEach((c) => { if (c.status === 'en_cours' || c.status === 'a_venir') parId[c.id] = c.client_name; });
          if (Object.keys(parId).length) {
            const j = (d) => new Date(d.getTime() - d.getTimezoneOffset() * 60000).toISOString().slice(0, 10);
            const d0 = new Date(); d0.setDate(d0.getDate() - 21);
            const d1 = new Date(); d1.setDate(d1.getDate() + 90);
            const taches = await sb.listTachesPeriode(j(d0), j(d1));
            out.etapes = (taches || [])
              .filter((t) => parId[t.chantier_id] && t.statut !== 'termine')
              .slice(0, 40)
              .map((t) => ({ titre: t.titre, client: parId[t.chantier_id], jour: t.jour || null, statut: t.statut || 'a_faire' }));
          }
        } catch (_) {}
    ...
    // (18/09) `opts.outil` : l'outil qu'une PUCE annonce exécuter. Il part en
    // `outil_attendu` et le serveur force Gemini à l'appeler. Il n'est JAMAIS
    // posé sur du texte libre — seul un clic sur une puce en fournit un.
    // Et il ne part que si le SUJET est résolu (voir bsAstOutilForcable) :
    // forcer `creer_facture` sur « Facture de solde » quand trois chantiers
    // sont à solder obligerait le modèle à en choisir un au hasard, et une
    // facture émise sur le mauvais chantier coûte de l'argent.
    async function bsAstAskLLM(question, bMsg, messages, opts) {
      // Poser une AUTRE question, c'est tourner la page : ce qui restait a
      // valider ne le sera plus. On le consigne comme un refus par inaction.
      bsAstSolderAbandons();
      ...
      const ctrl = new AbortController();
      // (12/09) Un couperet fixe à 25 s tuait les devis complets au démarrage à
      // froid (26 s mesurés à v159) : le test de fumée échouait trois fois de
      // suite sur « crée un devis… » sans rien dans le journal serveur, l'app
      // affichant « ne répond pas ». Désormais : 30 s d'INACTIVITÉ (remises à
      // zéro à chaque morceau reçu) et un plafond absolu de 90 s.
      let killer = setTimeout(() => ctrl.abort(), 30000);
      const plafond = setTimeout(() => ctrl.abort(), 90000);
      const encoreVivant = () => { clearTimeout(killer); killer = setTimeout(() => ctrl.abort(), 30000); };
      try {
        const headers = { 'content-type': 'application/json' };
        ...
        const r = await fetch(url, {
          ...
          body: JSON.stringify({
            ...
            prompt: question,
            messages: bsAstHistorique.map((m) => ({ role: m.role, content: m.content })),
            context,
            ...
            ...(opts && opts.outil ? { outil_attendu: opts.outil } : {}),
            // (19/09) ce que l'appli prévoyait — le serveur journalise l'écart.
            ...bsAstPrevuCorps(),
          }),
          signal: ctrl.signal,
        });
        ...
        if (r.status === 429) {
          ...
          if (err429 === 'plafond_cout' || err429 === 'quota_global') {
            // (11/09, Moctar : « ça va pas ce genre de message pour nos clients »)
            // Un plafond de coût est une affaire interne : l'artisan n'a pas à
            // lire « budget IA ». On dit l'indisponibilité, pas sa cause, et on
            // ne promet pas « demain matin » — le plafond peut être relevé avant.
            bMsg.innerHTML = "L'assistant est momentanément indisponible. Vos écrans restent utilisables ; réessayez dans un moment.";
            return;
          }
        ...
        bsAstHistPush('model', txt);
        bMsg.innerHTML = bsAstTexteRiche(txt);
        ...
        const props = (j && Array.isArray(j.actionProposals) && j.actionProposals.length)
          ? j.actionProposals
          : (j && j.actionProposal ? [j.actionProposal] : []);
        props.forEach((pr) => bsAstRendreProposition(pr, bMsg, messages, j.log_id, question, 0));
      } catch (e) {
        ...
        // Pas de « l'équipe est prévenue automatiquement » : les alertes sont
        // coupées jusqu'au lancement (05/09), la phrase était fausse (12/09).
        bMsg.innerHTML = "L'assistant ne répond pas pour l'instant. Réessayez dans un instant."
        ...
        clearTimeout(killer); clearTimeout(plafond);
        messages.scrollTop = messages.scrollHeight;
      }
    }
    ...
    async function sendMessage(text, opts) {
      ...
      try {
        ...
        // Puis les NOMS PROPRES, recalés sur ceux que l'artisan a vraiment.
        // C'est là que se logent les dernières erreurs de dictée : « Denali »
        // pour Benali, « Le Grand » pour Legrand. Une distance d'édition sur
        // une liste fermée ne peut pas inventer un nom, un modèle le pourrait.
        const an = dict.ancrerSurLesDonnees(q, dict.nomsConnus());
        if (an.corrections.length) {
          console.info('[dictée] noms recalés :', an.corrections.map((c) => `${c.avant} → ${c.apres}`).join(', '));
          q = an.texte;
        }
      ...
      couperDictee();
      if (typeof window.bsAstCouperDictees === 'function') window.bsAstCouperDictees();
      input.value = '';
      ...
      // serveur.
      bsAstAskLLM(q, bMsg, messages, opts);
    }

    sendBtn.addEventListener('click', () => sendMessage());
    input.addEventListener('keydown', (e) => {
      // (12/09) Entrée pendant une composition (dictée iOS, autocorrection) n'envoie pas :
      // « Rac » est parti seul à 13h05 chez Gabriel.
    ...
    if (SR) {
      try {
        ...
        recog.onend = () => {
          ...
          // On NE POSTE PAS tout seul à la fin de l'écoute. Même règle que la
          // dictée serveur (08/09) : le texte reste dans le champ, l'artisan
          // relit et envoie. Une transcription approximative partait sinon
          // sans qu'il ait rien vu.
          if (input.value.trim().length > 1) {
            if (!document.body.classList.contains('bs-demo-en-cours')) input.focus();
    ...
    function brancherMicServeur(brancher) {
      ...
      try {
        ...
        // (12/09, Moctar : « j'aimerais qu'il écrive en même temps que je dicte »)
        // Pendant l'enregistrement, la reconnaissance du NAVIGATEUR affiche le texte
        // en direct (approximatif, grisé) ; à l'arrêt, la transcription SERVEUR (qui
        // connaît le vocabulaire du bâtiment) le remplace. Si le navigateur refuse
        // (micro déjà pris), rien ne change : ancien comportement, sans erreur visible.
        let liveRecog = null; let liveAvant = null;
        // (12/09, retour d'Alice) Envoi pendant une dictée : le texte partait, puis la
        // transcription arrivait et RÉÉCRIVAIT le champ vidé. À l'envoi, on coupe la
        // dictée en direct et on ignore la prochaine transcription serveur.
        let ignorerProchaineFin = false;
        window.bsAstCouperDictees = () => {
          liveArreter(); liveAvant = null; input.classList.remove('bs-ast-live');
          if (micBtn.classList.contains('listening')) ignorerProchaineFin = true;
        };
        const SRLive = window.SpeechRecognition || window.webkitSpeechRecognition;
        function liveArreter() { if (liveRecog) { const r = liveRecog; liveRecog = null; try { r.onresult = null; r.onend = null; r.stop(); } catch (_) {} } }
        function liveDemarrer() {
          if (!SRLive || liveRecog) return;
          try {
            const r = new SRLive(); r.lang = 'fr-FR'; r.interimResults = true;
            r.continuous = !(/iPad|iPhone|iPod/.test(navigator.userAgent));
            liveAvant = input.value.trim();
            r.onresult = (ev) => {
              let t = '';
              for (let i = 0; i < ev.results.length; i++) t += ev.results[i][0].transcript + ' ';
              t = t.trim(); if (!t) return;
              input.value = (liveAvant ? liveAvant + ' ' : '') + t;
              input.classList.add('bs-ast-live');
            };
            r.onerror = () => { liveArreter(); };
            r.onend = () => { if (liveRecog === r && micBtn.classList.contains('listening')) { try { r.start(); } catch (_) {} } };
            r.start(); liveRecog = r;
          } catch (_) { liveRecog = null; }
        }
        try { const st = document.createElement('style'); st.textContent = '.bs-ast-input.bs-ast-live{color:#5A7268;font-style:italic}'; document.head.appendChild(st); } catch (_) {}
        brancher(micBtn, {
          contexte: 'assistant',
          texteAvant: () => input.value,
          // « relâchez pour ENVOYER » était faux depuis le 08/09 : le micro
          // écrit, il n'envoie plus. Et depuis le 09/09 un appui court suffit.
          onDebut: () => { micBtn.classList.add('listening'); input.placeholder = 'Je vous écoute…'; liveDemarrer(); },
          onVerrou: () => { input.placeholder = "J'enregistre — appuyez pour arrêter"; },
          onAttente: () => { input.placeholder = 'Je transcris…'; liveArreter(); },
          onFin: (res) => {
            micBtn.classList.remove('listening'); input.placeholder = phAvant;
            liveArreter();
            if (ignorerProchaineFin) { ignorerProchaineFin = false; liveAvant = null; input.classList.remove('bs-ast-live'); return; }
            const texte = res && typeof res.texte === 'string' ? res.texte.trim() : '';
            // Le texte « en direct » (navigateur) s'efface : la version serveur le remplace.
            if (liveAvant !== null) { input.value = liveAvant; liveAvant = null; input.classList.remove('bs-ast-live'); }
            // (25/09) Rien entendu ne doit JAMAIS être silencieux : l'artisan
            // appuie, parle, relâche, et si l'écran ne bouge pas il conclut que
            // l'outil est cassé. Le serveur dit toujours pourquoi.
            if (!texte) {
              const dit = (res && res.message) || "Je n'ai rien entendu, réessayez";
              input.placeholder = dit;
              if (typeof window.bsAssistantMessage === 'function') window.bsAssistantMessage(dit);
              setTimeout(() => { if (input.placeholder === dit) input.placeholder = phAvant; }, 6000);
              return;
            }
            // ON ÉCRIT, ON N'ENVOIE PAS (Moctar, 08/09 : « le micro n'écrit pas
            // le texte dicté »). C'était le comportement voulu au départ —
            // relâcher envoyait — mais une transcription est APPROXIMATIVE :
            // un nom de client mal entendu partait sans que l'artisan ait rien
            // vu, et il ne pouvait plus le rattraper. Il relit, corrige d'un
            // doigt, envoie. Un geste de plus, zéro question partie de travers.
            const avant = input.value.trim();
            input.value = avant ? `${avant} ${texte}` : texte;
            if (!document.body.classList.contains('bs-demo-en-cours')) input.focus();
            try { input.setSelectionRange(input.value.length, input.value.length); } catch (_) {}
            if (typeof input.dispatchEvent === 'function') input.dispatchEvent(new Event('input', { bubbles: true }));
            // Transcription rendue mais peu sûre : on le dit plutôt que de la
            // présenter comme certaine.
            if (res && res.doute && typeof window.bsAssistantMessage === 'function') {
              window.bsAssistantMessage(res.message
                || "Je ne suis pas sûr d'avoir tout compris — relisez avant de valider.");
            }
          },
          onErreur: (msg) => {
            micBtn.classList.remove('listening'); input.placeholder = msg || phAvant;
            liveArreter(); if (liveAvant !== null) { input.value = liveAvant; liveAvant = null; input.classList.remove('bs-ast-live'); }
            // (12/09) « le micro ne fonctionne pas » ×3 sans aucune trace serveur :
            // une erreur de dictée restait sur le téléphone. Remontée comme les
            // autres pannes (journal-erreurs), avec l'UA — c'est ce qui manque
            // pour diagnostiquer sans le téléphone en main.
            try { window.bsSignalerPanne && window.bsSignalerPanne({ action: 'dictee', mode: 'micro', code: 'erreur', detail: String(msg || 'erreur micro sans message') + ' | ' + navigator.userAgent.slice(0, 80) }); } catch (_) {}
          },
        });
        micBtn.style.touchAction = 'none';
        micBtn.title = 'Appuyer pour dicter (ou maintenir)';
    ...
    // (25/09/2026, Moctar : « quand j'enregistre la dictée rien ne se passe, il
    // n'écrit pas »). Reproduit : le bouton « Dicter un devis » annonçait
    // « Maintenir pour parler » et n'enregistrait RIEN. Il se contentait
    // d'ouvrir ce panneau. Aucun appel ne partait, le champ restait vide, et
    // l'artisan n'avait aucun moyen de comprendre pourquoi.
    // Le bouton ouvre maintenant le panneau ET lance l'écoute, exactement
    // comme si l'artisan avait appuyé lui-même sur le micro.
    window.bsAssistantDicterDevis = function (opts) {
      overlay.classList.add('open');
      const bMsg = document.createElement('div');
      bMsg.className = 'bs-ast-msg bot';
      bMsg.textContent = window.__bsMicServeurBranche
        ? 'Décrivez les travaux : appuyez sur le micro, parlez, appuyez à nouveau. Je prépare le devis avec vos prix, vous le relisez avant tout envoi.'
      ...
      // On déclenche le micro par un vrai geste sur le bouton : c'est le même
      // chemin que l'appui de l'artisan, donc le même comportement, la même
      // demande d'autorisation et le même verrouillage d'appui court.
      if (opts && opts.ecouter && window.__bsMicServeurBranche) {
        setTimeout(() => {
          try {
            ...
            micBtn.dispatchEvent(ev('pointerdown'));
            setTimeout(() => micBtn.dispatchEvent(ev('pointerup')), 120);
          } catch (_) { /* l'artisan garde le micro sous la main */ }
        }, 260);
      }
    };
    ...
    // Affiche un message de l'assistant SANS rien envoyer au modèle. Sert aux
    // retours de la dictée : « je n'ai rien entendu », « relisez avant de
    // valider ». Un écran muet fait croire que l'outil est cassé.
    window.bsAssistantMessage = function (texte) {
      const t = String(texte == null ? '' : texte).trim();
      if (!t) return;
      overlay.classList.add('open');
      const el = document.createElement('div');
      el.className = 'bs-ast-msg bot';
      el.textContent = t;
      messages.appendChild(el);
      messages.scrollTop = messages.scrollHeight;
    };

    window.bsAssistantEnvoyerQuestion = function (question, opts) {
    ...
    function bsAstPucesContextuelles() {
      ...
      // (19/09) Les actes prévus passent devant, quelle que soit la page (une
      // page qui a ses propres puces les garde derrière, dédoublonnées).
      {
        const prevues = bsPucesPrevues();
        if (prevues.length) {
          const vus = new Set(prevues.map((p) => p.label));
          sugg = prevues.concat((sugg || []).filter((s) => s && !vus.has(s.label))).slice(0, 4);
        }
      }
      ...
      sugg.slice(0, 4).forEach((s) => {
        ...
        c.addEventListener('click', () => {
          ...
          sendMessage(s.question || s.label, { outil: bsAstOutilForcable(s) });
        });
      ...
      if (!chipsContexteIntro) {
        ...
        // Fil repris (moins de 2 h) : on remet ses bulles, et une puce pour couper.
        if (bsAstHistorique.length && !messages.querySelector('.bs-ast-msg.user')) {
          bsAstHistorique.forEach((m) => {
            const d = document.createElement('div');
            d.className = 'bs-ast-msg ' + (m.role === 'user' ? 'user' : 'bot');
            if (m.role === 'user') d.textContent = m.content; else d.innerHTML = bsAstTexteRiche(m.affichage || m.content);
            messages.appendChild(d);
          });
          const coupe = document.createElement('button');
          coupe.type = 'button';
          coupe.className = 'bs-ast-chip';
          coupe.textContent = 'Nouvelle conversation';
          coupe.style.cssText = 'margin:4px 0 8px;font-size:12px;';
          coupe.addEventListener('click', () => window.bsAstNouvelleConversation());
          messages.appendChild(coupe);
          messages.scrollTop = messages.scrollHeight;
          return;
        }
        ...
        // (19/09) L'assistant parle le premier : ce que la base dit qu'il y a à
        // faire (js/app-prevision.js). Rien de prévu → l'accueil d'avant.
        // (24/09, Moctar : « il devrait dire bonjour et mieux présenter les choses à
        // faire ») : salutation selon l'heure et le prénom (fiche de candidature, lu
        // par app-prevision.js), puis UNE ligne par acte — le nom en gras, le devis et
        // la raison en clair — puis la question. Tout vient de la base, rien d'écrit
        // par le modèle : zéro appel, zéro invention.
        b.innerHTML = bsAstTexteRiche(bsAstMessageOuverture(ctx, ex));
  ...
  // (18/09) Chaque puce déclare l'outil qu'elle exécute et si cet outil a
  // besoin d'un sujet (client, chantier, devis, facture). `sujet: 'requis'`
  // sans `sujetResolu` = pas de forçage : la page n'a pas rempli le sujet,
  // l'assistant garde le droit de demander lequel. Voir bsAstOutilForcable.
  const BS_SUGGESTIONS_PAR_PAGE = {
    'devis.html': [
      { label: 'Relancer un devis en attente', outil: 'preparer_relance', sujet: 'requis' },
      { label: 'Planifier un devis signé', outil: 'planifier_depuis_devis', sujet: 'requis' },
      { label: 'Devis depuis une photo', photo: true },
    ],
    'dashboard.html': [
      { label: 'Où en est mon prochain chantier ?', outil: 'deroulement_chantier', sujet: 'aucun' },
      { label: 'Qui travaille demain ?', outil: 'qui_travaille', sujet: 'aucun' },
      { label: "Photo d'avancement", photo: true },
    ],
    'chantier.html': [
      // Le chantier est dans l'URL : chantier.html pose ses puces avec le
      // sujet déjà résolu (window.bsSuggestions). Ces libellés-ci ne servent
      // que le temps du chargement, donc sans forçage.
      { label: 'Décaler la prochaine étape', outil: 'deplacer_tache', sujet: 'requis' },
      { label: 'Prévenir le client', outil: 'envoyer_message_client', sujet: 'requis' },
      { label: 'Ajouter une étape', outil: 'ajouter_etape', sujet: 'requis' },
    ],
    'planning.html': [
      { label: 'Planifier un devis signé', outil: 'planifier_depuis_devis', sujet: 'requis' },
      { label: 'Qui est libre vendredi ?', outil: 'qui_travaille', sujet: 'aucun' },
      { label: 'Décaler une étape', outil: 'deplacer_tache', sujet: 'requis' },
    ],
    'photos.html': [
      { label: 'Envoyer les photos au client', outil: 'envoyer_photos_client', sujet: 'requis' },
      { label: 'Devis depuis cette photo', photo: true },
    ],
    'finances.html': [
      { label: 'Relancer une facture à encaisser', outil: 'preparer_relance', sujet: 'requis' },
      // (18/09) « Facture de solde » nu a été retiré : le 15/09 à 18h41 il a
      // produit une question (« pour quel chantier ? ») au lieu d'une facture,
      // parce que la puce ne porte aucun chantier — et trois étaient à solder.
      // L'onglet Factures pose déjà « Facture de solde {client} », nommée donc
      // exécutable. On ne propose pas une action qu'on n'a pas de quoi exécuter.
      { label: 'Export pour le comptable', outil: 'export_comptable', sujet: 'aucun' },
    ],
    'equipe.html': [
      { label: 'Ajouter un compagnon', outil: 'ajouter_coequipier', sujet: 'aucun' },
      { label: 'Charge de la semaine', outil: 'verifier_charge', sujet: 'aucun' },
    ],
  };;
  ...
  // ── Forçage d'outil : quand une puce peut annoncer ce qu'elle exécute ─────
  // (18/09) Une puce porte `outil` (ce qu'elle déclenche) et `sujet` :
  //   'aucun'  → l'outil n'a besoin d'aucune entité (« Qui travaille demain ? »)
  //   'requis' → il lui faut un client, un chantier, un devis ou une facture.
  // Le forçage ne part QUE si le sujet est résolu. L'étude du 18/09 a montré
  // que les deux tiers des échecs de l'assistant sont des puces au sujet vide :
  // l'assistant répond « pour quel chantier ? » parce que six sont candidats,
  // et il a raison. Forcer l'outil dans ce cas lui retirerait le droit de
  // demander — il devrait deviner, et une facture émise sur le mauvais
  // chantier coûte de l'argent. Une page résout l'entité et pose
  // `sujetResolu: true` sur sa puce (voir finances.html, onglet Factures).
  function bsAstOutilForcable(s) {
    if (!s || !s.outil || s.photo) return null;
    if (s.sujet === 'requis' && !s.sujetResolu) return null;
    return s.outil;
  }
    ...
    if (Array.isArray(window.bsSuggestions) && window.bsSuggestions.length) {
      return window.bsSuggestions
        ...
        .map((s) => ({ label: String(s.label), question: String(s.question || s.label), action: typeof s.action === 'function' ? s.action : null, photo: !!s.photo,
                       outil: s.outil || null, sujet: s.sujet || null, sujetResolu: !!s.sujetResolu }));
    }
    const cle = bsPageKey();
    const table = BS_SUGGESTIONS_PAR_PAGE[cle]
      || (BS_PAGES_GENERIQUES.has(cle) ? BS_SUGGESTIONS_GENERIQUES : null);
    const statiques = (table || []).map((s) => ({ label: s.label, question: s.question || s.label, photo: !!s.photo,
                                                 outil: s.outil || null, sujet: s.sujet || null, sujetResolu: !!s.sujetResolu }));
    // (19/09) La PRÉVISION passe devant la table statique : « Relancer Dupont
    // (devis 12) » est nommée, exécutable, et vient de l'état de la base
    // (js/app-prevision.js) ; « Relancer un devis en attente » ne l'est pas.
    // Une page qui pose ses propres puces (finances, chantier) garde la main :
    // on n'arrive ici que sans window.bsSuggestions.
    const prevues = bsPucesPrevues();
    if (!prevues.length) return table ? statiques : null;
    const vus = new Set(prevues.map((p) => p.label));
    return prevues.concat(statiques.filter((s) => !vus.has(s.label))).slice(0, 4);
  }
  /** Puces issues de la prévision (js/app-prevision.js), ou [] si le script
   *  n'est pas chargé, pas de session, ou rien de prévu. */
  function bsPucesPrevues() {
    try {
      return typeof window.bsPrevisionPuces === 'function'
        ? window.bsPrevisionPuces().filter((p) => p && p.label) : [];
    } catch (_) { return []; }
  }
  /** Le corps `prevu` joint à chaque requête : ce que l'appli prévoyait au
   *  moment où l'artisan a parlé (le serveur en journalise l'écart). */
  function bsAstPrevuCorps() {
    try {
      const p = typeof window.bsPrevisionOutils === 'function' ? window.bsPrevisionOutils() : null;
      return p && Array.isArray(p.actes) && p.actes.length ? { prevu: p } : {};
    } catch (_) { return {}; }
  }
  ...
  function bsTrouverMount() {
    // 25/09/2026 — le corps scrollable EN PREMIER. Sur les ecrans qui ont leur
    // propre carte (.mq-assist, Devis et Chantiers), celle-ci vit DANS
    // `.scr-body` et suit donc le contenu. La carte generique, elle, se posait
    // a cote : `.scr-body` porte `flex: 1`, il s'etirait sur toute la hauteur
    // et repoussait la carte tout en bas. Sur Finances cela laissait 174 px de
    // blanc entre le dernier bouton et la carte, alors que sur Devis elle
    // suivait la liste. Meme bloc, deux placements : c'est la meme incoherence
    // que celle des styles, vue par Moctar sur ses deux captures.
    return document.querySelector('.scr-body')
  ...
  function bsConstruireCarteSuggestions(suggestions) {
    ...
    // La marge horizontale est posee plus bas, une fois qu'on sait OU la carte
    // atterrit : dans `.scr-body` elle herite deja du retrait du conteneur, et
    // en rajouter 16 px la rendait plus etroite que les cartes voisines
    // (capture Moctar, 25/09). Ailleurs, elle doit poser son propre retrait.
    ...
    suggestions.slice(0, 3).forEach((s) => {
      ...
      chip.addEventListener('click', () => {
        ...
        if (s.photo) {
          ...
          window.bsAssistantEnvoyerQuestion(s.question || s.label, { outil: bsAstOutilForcable(s) });
        }
      });
  ...
  function bsInsererCarte(carte) {
    ...
    // Où poser la carte.
    //
    // Cas normal (25/09) : l'écran a un corps scrollable `.scr-body`. La carte
    // y entre en dernier, exactement comme la carte de la maquette sur Devis et
    // Chantiers. Elle suit le contenu au lieu d'être plaquée en bas : `.scr-body`
    // porte `flex: 1` et s'étire, donc une carte posée À CÔTÉ de lui se retrouvait
    // séparée du dernier bloc par toute la hauteur restante — 174 px de blanc
    // mesurés sur Finances.
    //
    // Sinon, on retombe sur l'ancien placement : juste avant la barre de
    // navigation, cherchée dans tout le document (audit 12/09, A45 — sur
    // certains écrans la nav vit hors du mount, et la carte partait après elle,
    // hors de vue).
    if (mount.classList && mount.classList.contains('scr-body')) {
      mount.appendChild(carte);
    } else {
      const nav = mount.querySelector('nav.bn') || document.querySelector('nav.bn') || document.querySelector('nav[role="navigation"]');
      if (nav && nav.parentNode) {
        nav.parentNode.insertBefore(carte, nav);
      } else {
        mount.appendChild(carte);
      }
    }
    // (24/09, capture Finances) Le bouton flottant de l'assistant (50 px, à 74 px du bas)
    // recouvrait la dernière proposition : la carte garde la place du bouton sous elle.
    try {
      carte.style.marginBottom = 'calc(84px + env(safe-area-inset-bottom, 0px))';
      // Alignement sur les cartes voisines : dans le corps scrollable, le
      // retrait lateral vient deja du conteneur. Hors de lui, la carte le pose.
      const dedans = mount.classList && mount.classList.contains('scr-body');
      carte.style.marginLeft = carte.style.marginRight = dedans ? '0px' : '16px';
    } catch (_) {}
  }
