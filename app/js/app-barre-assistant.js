...
function monter(h) {
  ...
  b.innerHTML = `<span class="bs-barre-ico">${window.bsIcon ? window.bsIcon('spark', 18) : ''}</span>
    ...
    <button type="button" class="bs-barre-btn bs-barre-mic" aria-label="Appuyer pour dicter (ou maintenir)">${window.bsIcon ? window.bsIcon('mic', 20) : ''}</button>
  ...
  if (dicteeServeurDisponible()) {
    brancherBoutonMaintenu(mic, {
      ...
      // Appui court : l'enregistrement continue sans le doigt (09/09).
      ...
      onFin: (res) => {
        ...
        // (25/09/2026, Moctar : « quand j'enregistre la dictée rien ne se
        // passe, il n'écrit pas ») Un texte vide ne peut PAS être traité en
        // silence. Le serveur dit toujours pourquoi — silence mesuré, rien
        // d'intelligible, appui trop court — et l'artisan a le droit de le
        // savoir. Sans ce retour, il appuie, parle, et croit l'outil cassé.
        if (!texte) {
          ...
          setTimeout(() => { if (input.placeholder === dit) input.placeholder = ph; }, 6000);
          return;
        }
        // On ÉCRIT dans le champ et on ouvre la conversation, on n'envoie pas :
        // l'artisan doit voir ce qui a été entendu avant que ça parte
        // (Moctar, 08/09). Même règle que le micro de la feuille.
        if (typeof window.bsAssistantOuvrirSaisie === 'function') {
          window.bsAssistantOuvrirSaisie(texte);
        } else {
          input.value = texte;
          input.focus();
        }
        // Transcription rendue mais peu sûre : on le dit, sans rien cacher.
