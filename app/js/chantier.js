...
import { normaliserHeure, creneauLisible } from './heures.js';
...
async function loadChantier() {
  ...
  renderHero();
  fillInfoForm();
  majSuggestionsChantier();
}

// (18/09) Les puces portent le NOM du chantier, pas un libellé creux.
// L'étude du 18/09 : deux tiers des échecs de l'assistant sont des puces au
// sujet vide — « Prévenir le client » sans client, et il répond « lequel ? ».
// Ici le chantier est dans l'URL : il n'y a aucune raison de le lui cacher.
// `sujetResolu` autorise app-assistant.js à forcer l'outil (bsAstOutilForcable).
function majSuggestionsChantier() {
  if (!chantier) return;
  const qui = chantier.client_name || 'le client';
  window.bsSuggestions = [
    { label: 'Décaler la prochaine étape', question: `Décale la prochaine étape du chantier de ${qui}`,
      outil: 'deplacer_tache', sujet: 'requis', sujetResolu: true },
    { label: `Prévenir ${qui}`, question: `Prépare un message pour prévenir ${qui}`,
      outil: 'envoyer_message_client', sujet: 'requis', sujetResolu: true },
    { label: 'Ajouter une étape', question: `Ajoute une étape au chantier de ${qui}`,
      outil: 'ajouter_etape', sujet: 'requis', sujetResolu: true },
  ];
...
function formatDatesChantier(c) {
  ...
  // (A41, audit 12/09) La fin se déduit des ÉTAPES quand elles existent : la
  // fin prévue saisie (« 13 sept. ») contredisait le déroulement (dernière
  // étape le 6). Sans étape datée, on garde la fin prévue saisie.
  let finEtapes = null;
  try {
    if (typeof dernieresTaches !== 'undefined' && Array.isArray(dernieresTaches)) {
      dernieresTaches.forEach((t) => { const f = t.jour_fin || t.jour; if (f && (!finEtapes || f > finEtapes)) finEtapes = f; });
    }
  } catch (_) {}
  const d2 = finEtapes ? fmtDate(finEtapes) : (c.date_fin_prevue ? fmtDate(c.date_fin_prevue) : null);
...
// téléphone propose (Android liste Maps ET Waze). Le bouton « répertoire »
// (fiche vCard) a été retiré le 07/09 : appeler pose déjà le numéro sur le
// téléphone, d'où on l'enregistre si on veut.
function renderContactActions() {
  ...
  // (Moctar, 07/09) « Répertoire inutile : quand on clique sur Appeler, le
  // numéro est sur le téléphone, on peut appeler ensuite. » C'était vrai, et le
  // bouton posait en plus un vrai défaut sur Android (il téléchargeait un
  // fichier .vcf). À sa place, ce qui manquait : dire QUI de l'équipe est sur
  // ce chantier.
  {
    const b = document.createElement('button');
    b.type = 'button'; b.className = 'mq-btn mq-ghost';
    b.title = 'Ajouter des compagnons à ce chantier';
    b.innerHTML = ico('users') + '<span style="font-size:16px;line-height:1;margin-left:2px;">＋</span>';
    b.setAttribute('aria-label', 'Ajouter des compagnons à ce chantier');
    b.addEventListener('click', ouvrirAjoutCompagnons);
    zone.appendChild(b);
  }
  // (Moctar, 08/09) « dans chantier, l'encadre qui regroupe le chantier, etape
  // et date : appeler le client, nouveau devis, message ». Appeler et
  // itineraire existaient ; il manquait les deux gestes qui SUIVENT une visite
  // de chantier — chiffrer un travail en plus, et prevenir le client.
  {
    // Un travail en plus se chiffre a part : au forfait, l'article 1793 du
    // Code civil n'autorise un supplement que s'il est ecrit et le prix
    // convenu. On ouvre donc un devis NEUF, jamais une version du devis signe.
    const d = document.createElement('a');
    d.className = 'mq-btn mq-ghost';
    // Le nom du client voyage avec : devis.html ouvre l'assistant dessus
    // (« Nouveau devis pour Mme Ravier… »), sans refaire une lecture.
    d.href = `./devis.html?chantier=${encodeURIComponent(chantier.id)}&client=${encodeURIComponent(chantier.client_name || '')}`;
    d.title = 'Nouveau devis pour ce chantier (travaux supplémentaires)';
    d.setAttribute('aria-label', 'Nouveau devis pour ce chantier');
    d.innerHTML = ico('doc') + '<span style="margin-left:4px">Devis</span>';
    zone.appendChild(d);
  }
  // Le bouton « Message » de cette barre doublait l'onglet Messages du même
  // écran (Moctar, 11/09 07h09 : « 2 boutons messages »). L'onglet suffit.
  zone.hidden = !zone.children.length;
}

// ── Qui de l'équipe est sur ce chantier ───────────────────────────────
// Moctar, 07/09 : « dans Clients, en cliquant sur un client, l'artisan peut
// décider d'ajouter des membres de son équipe. »
//
// Ce que ça fait, et ce que ça ne fait pas — dit tel quel dans la feuille :
// un compagnon apparaît sur un chantier parce qu'il y a une ÉTAPE assignée
// (table `taches`). Annoncer quelqu'un ici le prévient dans le fil du chantier
// et le désigne comme concerné ; ça ne lui invente pas de travail. Pour qu'il
// apparaisse au planning, on lui donne une étape — le lien est dans la feuille.
async function ouvrirAjoutCompagnons() {
  let membres = [], taches = [];
  try {
    [membres, taches] = await Promise.all([
      listMembres().catch(() => []),
      listTachesChantier(chantier.id).catch(() => []),
    ]);
  } catch (_) { /* on ouvre quand même, la feuille le dira */ }
  const equipe = (membres || []).filter((m) => m.statut !== 'inactif' && m.prenom);
  const dejaLa = new Set((taches || []).map((t) => t.assigne_a).filter(Boolean));

  const fond = document.createElement('div');
  fond.style.cssText = 'position:fixed;inset:0;background:rgba(15,81,50,.45);z-index:1200;'
    + 'display:flex;align-items:flex-end;justify-content:center;';
  const f = document.createElement('div');
  f.style.cssText = 'background:#fff;width:100%;max-width:480px;border-radius:16px 16px 0 0;'
    + 'padding:18px 16px 24px;max-height:82vh;overflow:auto;';
  fond.appendChild(f);
  fond.addEventListener('click', (e) => { if (e.target === fond) fond.remove(); });

  const t = document.createElement('div');
  t.style.cssText = 'font-weight:800;font-size:17px;margin-bottom:4px;';
  t.textContent = 'Compagnons sur ce chantier';
  const st = document.createElement('div');
  st.style.cssText = 'color:#5A7268;font-size:13.5px;margin-bottom:14px;';
  st.textContent = equipe.length
    ? "Les compagnons cochés sont prévenus dans le fil du chantier. Pour qu'ils apparaissent au planning, donnez-leur une étape."
    : "Vous n'avez pas encore de compagnon. Invitez-en un depuis Équipe.";
  f.append(t, st);
  ...
  equipe.forEach((m) => {
    ...
    l.style.cssText = 'display:flex;align-items:center;gap:10px;min-height:46px;padding:0 12px;'
      + 'border:1.5px solid #D3E3DA;border-radius:12px;font-size:15px;cursor:pointer;';
    const cb = document.createElement('input');
    cb.type = 'checkbox'; cb.value = m.prenom; cb.checked = dejaLa.has(m.prenom);
    cb.style.cssText = 'width:20px;height:20px;accent-color:#228B5B;';
    const sp = document.createElement('span');
    sp.textContent = m.prenom + (dejaLa.has(m.prenom) ? ' · déjà sur le chantier' : '');
    l.append(cb, sp);
    zone.appendChild(l);
  });
  f.appendChild(zone);
  ...
  if (equipe.length) {
    ...
    ok.type = 'button'; ok.className = 'ui-primary';
    ok.style.cssText = 'width:100%;margin-top:16px;min-height:48px;font-size:15.5px;';
    ok.textContent = 'Prévenir dans le fil du chantier';
    ok.addEventListener('click', async () => {
      const noms = Array.from(zone.querySelectorAll('input:checked')).map((c) => c.value);
      if (!noms.length) { fond.remove(); return; }
      ok.disabled = true; ok.textContent = 'Envoi…';
      try {
        // envoyerMessageEquipe et PAS une insertion brute : elle renseigne
        // `author` et `auteur_id`. Un message sans auteur est compte comme non
        // lu et ne peut JAMAIS etre marque lu — c'est ce qui bloquait la
        // pastille a 1. Ne jamais ecrire dans `messages` a la main.
        await envoyerMessageEquipe(chantier.id,
          noms.join(', ') + (noms.length > 1 ? ' sont' : ' est')
          + ' sur le chantier ' + (chantier.client_name || '') + '.');
        toast('Compagnons prévenus.');
        fond.remove();
      } catch (e) {
        ok.disabled = false; ok.textContent = 'Prévenir dans le fil du chantier';
        toast("L'annonce n'a pas pu être envoyée.");
      }
    });
    f.appendChild(ok);
  }
  const an = document.createElement('button');
  an.type = 'button'; an.className = 'mq-btn mq-ghost';
  an.style.cssText = 'width:100%;margin-top:12px;min-height:44px;';
  an.textContent = 'Fermer';
  an.addEventListener('click', () => fond.remove());
  f.appendChild(an);
...
function majEnTeteAvancement(taches) {
  // (A41) Les dates de l'en-tête suivent les étapes : on les recalcule ici.
  try {
    const sous = document.getElementById('mq-sous');
    if (sous && chantier) {
      const segs = [];
  ...
  // Même règle que le tableau de bord : une étape datée AVANT le démarrage du
  // chantier est MAL DATÉE, pas en retard. La compter en retard produisait
  // « du 8 sept. au 15 sept. · dans 7 j » à côté de « Retard 19 j » — deux
  // affirmations incompatibles (Moctar, 08/09).
  const debutChantier = chantier && chantier.date_debut
    ? String(chantier.date_debut).slice(0, 10) : null;
  let malDatees = 0;
  let retard = 0;
  for (const t of taches) {
    ...
    if (!limite) continue;
    if (debutChantier && String(limite).slice(0, 10) < debutChantier) { malDatees++; continue; }
    if (limite >= aujourdhui) continue;
  ...
  const statutTxt = retard > 0
    ? `retard ${retard} j`
    : (malDatees ? `${malDatees} étape${malDatees > 1 ? 's' : ''} datée${malDatees > 1 ? 's' : ''} avant le démarrage` : 'dans les temps');
  // UNE phrase, en toutes lettres. Deux pourcentages côte à côte demandent au
  // lecteur de faire la comparaison lui-même — et Moctar, trois fois le 08/09,
  // a dit qu'il ne la faisait pas : « je comprends toujours pas », « la barre
  // verte est plus loin que là où on en est », « il faut une barre plus simple ».
  // La barre dit CE QUI EST FAIT, comme partout ailleurs. Le verdict est écrit.
  ligne.textContent = `${faites} étape${faites > 1 ? 's' : ''} sur ${total} · ${statutTxt}`;
...
async function toggleEtapeCoche(cbBtn, t) {
  ...
  try {
    ...
    // On coche SUR PLACE. Avant, un simple clic appelait renderDeroulement(),
    // qui vidait la liste pour y poser un squelette de trois lignes, refaisait
    // une requête réseau et reconstruisait tout le DOM : la liste sautait, la
    // position de lecture était perdue, et les animations se rejouaient. C'est
    // le « glitch » signalé par Moctar le 08/09. Cocher une case ne change ni
    // l'ordre ni le regroupement par jour : rien ne justifie de tout refaire.
    const ligne = cbBtn.closest('.mq-task');
    if (ligne) {
      ligne.classList.toggle('mq-done', versTermine);
      // Une étape terminée n'est plus en retard : on retire l'ambre posé en
      // ligne au rendu, sinon elle reste barrée ET rouge.
      if (versTermine) ligne.style.color = '';
    }
    cbBtn.setAttribute('aria-checked', String(versTermine));
    const pastille = cbBtn.querySelector('.mq-cb');
    if (pastille) pastille.classList.toggle('mq-done', versTermine);
    cbBtn.disabled = false;
    // L'en-tête, lui, doit suivre : c'est la même lecture des tâches, en
    // mémoire, sans requête.
    const memo = dernieresTaches.find((x) => x.id === t.id);
    if (memo) memo.statut = nouveauStatut;
    majEnTeteAvancement(dernieresTaches);
    renderStatutCalcule(dernieresTaches);
  } catch (err) {
...
function ouvrirFormEtape(ancre, tache) {
  ...
  ligne.append(
    ...
    // Moctar, 07/09 : « certains métiers ont plusieurs interventions par
    // jour ». L'heure est FACULTATIVE : un chantier au forfait se planifie à
    // la journée et n'en veut pas ; un dépannage en veut une. Le champ ne
    // porte donc aucune valeur par défaut — un « 08:00 » posé d'office
    // écrirait une heure que personne n'a décidée.
    // (#4, Moctar 09/09 : « pour la sélection d'heure, pas l'horloge, format
    // digital ») : un `type="time"` ouvre le cadran sur Android. Un champ
    // texte numérique « 08:30 » se tape en quatre chiffres ; « 8h30 » et
    // « 0830 » sont acceptés et normalisés à l'enregistrement.
    champ('df-heure', 'Heure (facultatif)', 'text', tache ? normaliserHeure(tache.heure_debut) : '',
          { inputmode: 'numeric', placeholder: '08:30', maxlength: '5', autocomplete: 'off',
            pattern: '^([01]?[0-9]|2[0-3])[:hH]?[0-5][0-9]$', title: 'Heure au format 08:30' }),
    champ('df-duree', 'Durée (h)', 'number', tache ? tache.duree_h : '', { min: '0.25', step: '0.25' }),
  );
  const noteHeure = document.createElement('div');
  noteHeure.style.cssText = 'font-size:12.5px;color:#6B7F76;margin-top:-4px;';
  noteHeure.textContent = 'Sans heure, l’étape occupe la journée. Avec une heure, elle devient un créneau — plusieurs interventions peuvent tenir dans la même journée.';
  ...
  f.append(titre, desc, ligne, noteHeure, boutons);
  ...
  f.addEventListener('submit', async (e) => {
    ...
    const champs = {
      ...
      // Champ vidé = `null`, pas `undefined` : c'est une demande explicite de
      // repasser l'étape à la journée entière.
      heure_debut: (() => {
        const m = String(f.querySelector('#df-heure').value || '').trim().match(/^(\d{1,2})[:hH]?(\d{2})$/);
...
function zonePlanification() {
  ...
  champ.min = new Date(Date.now() - new Date().getTimezoneOffset() * 60000).toISOString().slice(0, 10);
  ...
  const note = document.createElement('div');
  note.style.cssText = 'font-size:12.5px;color:#6B7F76;';
...
async function renderDeroulement() {
  ...
  // La question que l'artisan se pose en ouvrant l'ecran le matin, c'est
  // « je suis ou, la ? ». Le deroulement montrait l'ORDRE des etapes, jamais
  // la position du JOUR. On pose donc un trait « aujourd'hui » a sa place
  // exacte dans la suite des jours (idee retenue avec Moctar, 08/09).
  const AUJ = new Date(); AUJ.setHours(0, 0, 0, 0);
  const cleAuj = AUJ.getFullYear() + '-' + String(AUJ.getMonth() + 1).padStart(2, '0')
                 + '-' + String(AUJ.getDate()).padStart(2, '0');
  const joursPresents = Array.from(parJour.keys());
  let traitPose = false;
  const poserTraitAujourdhui = () => {
    if (traitPose) return;
    traitPose = true;
    const t = document.createElement('div');
    t.className = 'mq-auj';
    const e = document.createElement('span');
    // (15/09, Moctar) Sans date, ce trait — dessine comme un titre de section —
    // se lisait comme le titre du jour affiche juste en dessous.
    e.textContent = libelleAujourdhui(cleAuj);
    t.appendChild(e);
    liste.appendChild(t);
  };

  for (const [jour, lot] of parJour) {
    // (15/09, Moctar) Le trait ne se pose QUE si une etape tombe le jour
    // meme. Dans une periode creuse il se retrouvait colle au-dessus d'un jour
    // futur et, dessine comme un titre de rubrique, se lisait comme le titre
    // de ce jour-la — dater le libelle n'y avait rien change.
    if (poserTraitIci(joursPresents, cleAuj, jour)) poserTraitAujourdhui();
    ...
    if (jour === 'sans-date') {
      ...
      // (A39, audit 12/09) : « Mar 1 » sans mois est ambigu dès que le
      // chantier chevauche deux mois — le mois, en petit, sous le numéro.
      const moisEl = document.createElement('small');
      moisEl.className = 'mq-d-mois';
      moisEl.textContent = new Date(jour + 'T00:00:00').toLocaleDateString('fr-FR', { month: 'short' }).replace('.', '');
      dEl.appendChild(moisEl);
    }
    ...
    for (const t of lot) {
      ...
      // Un SECHAGE n'est pas une tache : c'est du temps mort obligatoire,
      // pendant lequel personne ne travaille. Affiche comme les autres, il
      // faisait croire a une journee d'equipe — ou pire, poussait a poser
      // trop tot sur un support humide (Moctar, 08/09). On le distingue :
      // pas d'assigne ET un intitule de temps de prise.
      const ATTENTE = /s[ée]chage|durcissement|prise\b|repos|attente|d[ée]lai|cure\b|temps de/i;
      const estAttente = ATTENTE.test(t.titre || '')
        && !(t.assignes && t.assignes.length)
        && !t.assigne_a;

      const row = document.createElement('div');
      row.className = 'mq-task' + (t.statut === 'termine' ? ' mq-done' : '')
        + (estAttente ? ' mq-attente' : '');
      ...
      // Le créneau devant l'intitulé quand il y en a un : « 8 h → 10 h ·
      // Dépannage chaudière ». Sans heure, rien ne s'affiche — l'étape occupe
      // la journée, et un « — » de plus n'apprendrait rien.
      const creneau = creneauLisible(t.heure_debut, t.duree_h);
      if (creneau) {
        const h = document.createElement('span');
        h.className = 'mq-creneau';
        h.textContent = creneau;
        row.appendChild(h);
      }
...
async function renderPhotos({ silencieux = false } = {}) {
  ...
  // (A64, audit 12/09) : ouvert sur Photos, l'en-tête restait sur
  // « Avancement… » — la même lecture des tâches remplit l'en-tête ici aussi.
  try { majEnTeteAvancement(dernieresTaches); renderStatutCalcule(dernieresTaches); } catch (_) {}
  ...
  // (A65, audit 12/09) : sans photo, « Envoyer au client », « Devis depuis
  // cette photo », « Comparer » sont trois actions impossibles — les puces
  // suivent l'état réel de la galerie.
  if (ongletCourant === 'photos') {
    const zoneChips = document.getElementById('mq-chips');
    if (zoneChips) poserChips(zoneChips, photosCourantes.length ? CHIPS_PAR_ONGLET.photos : ['Photo de début de chantier', "Photo d'avancement"]);
  }
  renderCasesTemps();
...
async function renderMessages() {
  ...
  try {
    ...
    // UN SEUL nouvel essai, tout de suite, et seulement si c'est le RÉSEAU qui
    // a lâché : une 4G de chantier perd une requête sans que rien ne soit
    // cassé, et faire réessayer l'artisan à la main pour ça n'a aucun sens.
    // Une erreur de droits ou de données, elle, se reproduira à l'identique :
    // on ne la rejoue pas.
    const reseau = err1 && (err1.delaiDepasse || /fetch|network|réseau|reseau|load failed/i.test(err1.message || ''));
    if (!reseau) { msgs = null; erreurFil = err1; }
    else {
      try {
        msgs = await listMessages(chantierId);
      } catch (err2) { msgs = null; erreurFil = err2; }
    }
  }
  if (msgs === null) {
    const err = erreurFil;
    // On JOURNALISE la cause : sans elle, « indisponible » est un mur, et on ne
    // peut rien diagnostiquer (Moctar, 08/09 : « message indisponible dans
    // l'outil »). Et on donne un vrai bouton Réessayer plutôt qu'une consigne.
    console.error('[messages] chargement impossible', err);
    // On ÉCRIT la cause. « Indisponible » tout court ne se diagnostique pas :
    // c'est ce mur qui m'a fait traiter le symptôme deux fois (08/09). Quatre
    // causes, quatre phrases — celle qui s'affiche dit quoi chercher.
    const msg = String((err && err.message) || '');
    const texte = !navigator.onLine
      ? 'Pas de réseau : les messages se chargeront à la reconnexion.'
      : (err && err.delaiDepasse)
        ? 'Le fil met trop de temps à venir (plus de 15 s). Réessayez : c’est en général le réseau du chantier.'
        : /jwt|expired|401/i.test(msg)
          ? 'Votre session a expiré. Rechargez la page pour vous reconnecter.'
          : /permission|policy|row-level|403/i.test(msg)
            ? 'Ce fil ne vous est pas accessible. Signalez-le : ce n’est pas normal.'
            : `Les messages n’ont pas pu être chargés (${msg.slice(0, 80) || 'cause inconnue'}).`;
    const bloc = rendreEtatVide(texte);
    const b = document.createElement('button');
    b.type = 'button'; b.className = 'mq-btn mq-ghost';
    b.style.cssText = 'margin-top:10px;min-height:44px;';
    b.textContent = 'Réessayer';
    b.addEventListener('click', () => renderMessages());
    bloc.appendChild(b);
    list.replaceChildren(bloc);
    return;
  }
...
brancherComposeur({
  ...
  onEnvoyer: async () => {
    ...
    try {
      ...
      msgInput.value = '';
      renderMessages();
      // (12/09, Moctar) Sans e-mail, le client n'a jamais reçu de lien : il ne verra
      // pas ce message. On le dit au moment de l'envoi, pas trois jours plus tard.
      if (typeof chantier !== 'undefined' && chantier && !chantier.client_email) {
        toast(`${chantier.client_name || 'Le client'} n'a pas d'e-mail : il ne verra pas ce message. Ajoutez son e-mail (bouton « E-mail à ajouter » en haut de la fiche).`, 'warning');
      }
  ...
  try {
    ...
    if (docs.length === 0) {
      ...
      docs.forEach(doc => {
        ...
        const meta = makeEl('div', 'font-size:12.5px;color:#9CA3AF;margin-top:1px');
        ...
        const dlBtn = makeEl('button', 'padding:5px 10px;font-size:12.5px;white-space:nowrap;flex-shrink:0');
        ...
        if (doc.uploader_id === session.user.id) {
          const delBtn = makeEl('button', 'padding:5px 8px;font-size:12.5px;flex-shrink:0');
    ...
    [['devis','Devis'],['facture','Facture'],['autre','Autre']].forEach(([val, label], i) => {
      const chip = makeEl('span', 'font-size:12.5px;font-weight:700;padding:4px 10px;border-radius:12px;cursor:pointer;color:#374151;border:1.5px solid #E5E7EB', label);
