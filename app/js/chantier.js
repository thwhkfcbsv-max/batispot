...
function renderContactActions() {
  ...
  // de chantier — chiffrer un travail en plus, et prevenir le client.
  {
    ...
    d.href = `./devis.html?chantier=${encodeURIComponent(chantier.id)}`;
...
async function renderDeroulement() {
  ...
  const poserTraitAujourdhui = () => {
    ...
    e.textContent = "aujourd'hui";
    t.appendChild(e);
    liste.appendChild(t);
  };

  for (const [jour, lot] of parJour) {
    // Le trait se pose AVANT le premier jour a venir. Un jour egal a
    // aujourd'hui reste sous le trait : la journee n'est pas finie.
    if (jour !== 'sans-date' && jour > cleAuj) poserTraitAujourdhui();
