// Un devis signé porte 'accepte' EN BASE — la contrainte CHECK de la table
// refuse toute autre valeur (vérifié le 08/09). 'signe' est le statut de
// l'INTERFACE : devis-store.js traduit 'accepte' en 'signe' pour l'écran
// (BASE_VERS_UI), et devis.html raisonne sur cette forme-là.
//
// Les deux formes circulent donc dans l'application selon d'où viennent les
// objets : listerDevis() rend les lignes brutes, chargerDevisUI() les rend
// traduites. Un filtre qui n'accepte qu'une des deux formes marche par
// coïncidence — il casse le jour où l'appelant change de source. On accepte
// les deux, une fois, ici, plutôt que de refaire ce pari à chaque appel.
export const estSigne = (q) => q && (q.status === 'accepte' || q.status === 'signe');
...
export function marges(zone, acceptes, dep, limite = 5) {
  ...
  // (12/09, Moctar : « marge par chantier en pourcentage, j'ai aucun repere »)
  // L'appelant passe les devis acceptes ET les depenses DU CHANTIER EN ENTIER
  // (pas ceux de la periode) : un chantier de 8 909 EUR avec 533 EUR d'achats
  // sur le mois affichait 94 %, alors que ses achats totaux donnent 83 %.
  ...
  // Reperes : ce que le % veut dire, et la moyenne de SES chantiers.
  const caTot = lignes.reduce((s, c) => s + c.ca, 0);
  const depTot = lignes.reduce((s, c) => s + c.dep, 0);
  const moyenne = Math.round(((caTot - depTot) / caTot) * 100);
  const entete = document.createElement('p');
  entete.style.cssText = 'margin:0 0 10px;font-size:12.5px;color:#3D5A4E;line-height:1.45;';
  entete.textContent = 'Part du devis HT qui reste une fois les achats payes (materiaux, outillage, sous-traitance). '
    + "Elle doit couvrir la main-d'oeuvre et vos frais. Le trait pointille = votre moyenne, " + moyenne + ' %.';
  ...
  lignes.forEach((c) => {
    ...
    haut.style.cssText = 'display:flex;justify-content:space-between;align-items:baseline;gap:10px;font-size:12.5px;margin-bottom:4px;';
    const nom = document.createElement('span');
    ...
    detail.style.cssText = 'font-size:12.5px;color:#5B7266;font-variant-numeric:tabular-nums;margin-bottom:5px;';
    detail.textContent = `${eur(c.ca)} de devis − ${eur(c.dep)} d'achats = ${eur(c.ca - c.dep)}`;
    const piste = document.createElement('div');
    piste.style.cssText = 'position:relative;height:10px;background:#EEF3F0;border-radius:99px;overflow:visible;';
    ...
    repere.title = `Votre moyenne : ${moyenne} %`;
    repere.style.cssText = `position:absolute;top:-3px;bottom:-3px;left:${Math.max(Math.min(moyenne, 100), 0)}%;`
      + 'width:0;border-left:2px dotted #1F2933;';
    piste.append(jauge, repere);
    bloc.append(haut, detail, piste);
    zone.appendChild(bloc);
  });
  const pied = document.createElement('p');
  pied.style.cssText = 'margin:8px 0 0;font-size:12.5px;color:#5B7266;line-height:1.45;';
  pied.textContent = 'En orange : moins de 20 %, les achats mangent presque tout. Devis et achats du chantier pris en entier, quelle que soit la periode.';
...
export function devis(zone, kpis, liste) {
  ...
  // 'accepte' compte AUSSI les 'signe' : deux chemins écrivent la signature
  // (voir estSigne en haut de ce fichier). Sans ça, la barre « acceptés » et
  // le taux d'acceptation sous-comptaient tous les devis cochés à la main.
  const compte = (st) => liste.filter((d) => (st === 'accepte' ? estSigne(d) : d.status === st)).length;
  ...
  if (kpis) {
    const tuile = (libelle, valeur, detail) => {
      ...
      if (detail) {
        ...
        d.style.cssText = 'font-size:12.5px;color:#5A7268;'; d.textContent = detail;
        b.appendChild(d);
      }
      return b;
    };
...
export function clients(zone, badge, acceptes, limite = 6) {
  ...
  if (liste.length > 1 && part >= 40) {
    ...
    note.style.cssText = 'font-size:12.5px;color:#B45309;margin:2px 0 0;font-weight:600;';
