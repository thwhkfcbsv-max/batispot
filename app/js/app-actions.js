...
async function envoyerDevis(p) {
  ...
  // Le devis part chez le client : l'artisan se sert vraiment de l'outil.
  try {
    const { devisEnvoyeClient } = await import('./conversion.js');
    devisEnvoyeClient('lien');
  } catch (_) {}
...
// « Mme Ravier a refusé le devis », « le client ne donne pas suite »,
// « classe le devis Jaurès en refusé » (13/09/2026, demande Moctar).
//
// Un devis refusé ne s'efface pas : il garde son numéro et sa place dans la
// série, et il rejoint le tiroir « Refusés / expirés ». C'est ce qui permet,
// six mois plus tard, de savoir combien on en a perdu et pourquoi — le motif
// est la seule information qui a de la valeur ici, on la demande donc.
async function marquerDevisRefuse(p) {
  const devis = await chargerDevis({});
  const d = refDevis(devis, p.devis_ref, p.client_nom);
  if (!d) throw new Error(`Devis introuvable pour « ${p.devis_ref || p.client_nom || '?'} ».`);

  const client = (d.chantiers && d.chantiers.client_name) || 'le client';
  if (d.status === 'refuse') {
    return { message: `Le devis ${d.numero} était déjà classé refusé.`, lien: './devis.html' };
  }
  if (d.status === 'brouillon') {
    throw new Error(`Le devis ${d.numero} n'est jamais parti chez ${client} : un brouillon se supprime, il ne se refuse pas.`);
  }
  if (d.status === 'accepte') {
    throw new Error(`Le devis ${d.numero} a été accepté par ${client} : il ne peut pas être classé refusé. Si les conditions changent, faites-en une nouvelle version.`);
  }

  const motif = String(p.motif || '').trim();
  const maj = { status: 'refuse' };
  if (motif) {
    // On lit les notes avant d'écrire : y ajouter une ligne datée, jamais
    // remplacer ce que l'artisan avait noté.
    const { data: avant } = await supabase.from('devis').select('notes').eq('id', d.id).single();
    const ancien = (avant && avant.notes) ? String(avant.notes).trim() : '';
    const ligne = `Refusé le ${new Date().toLocaleDateString('fr-FR')} — ${motif}`;
    maj.notes = ancien ? ancien + '\n' + ligne : ligne;
  }

  const { error } = await supabase.from('devis').update(maj).eq('id', d.id);
  if (error) throw error;

  return {
    message: `Devis ${d.numero} classé refusé${motif ? ' — ' + motif : ''}.`
      + ` Il reste dans vos devis, dans le tiroir « Refusés / expirés ».`,
    lien: './devis.html',
  };
}
...
async function releverPiece(p) {
  ...
  // (19/09, Moctar) Plus de repli « photo + feuille A4 » : une cote estimée
  // n'irait pas dans un devis. Un téléphone qui ne mesure pas le dit, et garde
  // le reste de Studio.
  if (cap.methode === 'aucune' || cap.methode === 'photo') {
    return {
      message: "Le metre n'est pas disponible sur ce telephone : il demande la realite "
             + "augmentee (Chrome sur un Android compatible, ou l'application BatiSpot sur "
             + "iPhone). Vous pouvez photographier un ticket depuis Finances, ou ajouter des "
             + "photos de chantier.",
    };
  }
  // Sur le web (Chrome Android + ARCore), le viseur vit dans metre.html : on
  // l'ouvre. Le plugin natif n'existe que dans l'application iPhone.
  if (!cap.natif) {
    window.location.href = './metre.html';
    return { message: "J'ouvre le metre." };
  }
...
// Le XML Factur-X d'une facture existante, depuis les vraies données : le
// profil de l'artisan (émetteur), le chantier (client), la facture (lignes).
// Ne transmet rien — produire le fichier et le transmettre sont deux métiers,
// et seul le premier ne dépend de personne (13/09/2026).
export async function facturXDeFacture(factureId) {
  const FX = await import('./factur-x.js');
  const { data: f, error } = await supabase.from('factures')
    .select('*, chantiers(client_name, adresse, client_email, client_est_pro, client_siren)')
    .eq('id', factureId).single();
  if (error) throw error;

  const { data: proId } = await supabase.rpc('mon_entreprise');
  const { data: profil } = await supabase.from('pro_profiles').select('*').eq('pro_id', proId).single();

  const ch = f.chantiers || {};
  const acheteur = {
    nom: ch.client_name, adresse: ch.adresse, email: ch.client_email,
    est_pro: !!ch.client_est_pro, siren: ch.client_siren || null,
  };
  // `notif_email` est un BOOLEEN (l'artisan veut-il des notifications), pas une
  // adresse : le passer en `email` posait `true` dans le XML. La seule adresse
  // fiable de l'artisan est celle de son compte.
  const { data: { user } } = await supabase.auth.getUser();
  const vendeur = { ...(profil || {}), email: user?.email || null };

  // Les lignes de BatiSpot ne portent pas de taux de TVA — il ne vit qu'au niveau
  // de la facture. EN16931 l'exige par ligne : on le déduit des totaux, et on ne
  // l'invente pas si la déduction ne tombe pas sur un taux légal (facture à
  // plusieurs taux). `verifier` le dit alors à l'artisan.
  const opts = { tauxTva: FX.tauxDeduit(f.total_ht, f.total_tva) ?? undefined };
  const manques = FX.verifier(f, vendeur, acheteur, opts);
  return { xml: FX.construireXML(f, vendeur, acheteur, opts), manques, nomFichier: FX.NOM_FICHIER };
}

// ------------------------------------------------- plateforme agréée

// « Je n'ai pas de plateforme » (13/09/2026, demande Moctar : « je facilite la
// vie de ceux qui n'ont pas encore de compte »).
//
// Depuis le 01/09/2026 une entreprise doit pouvoir RECEVOIR ses factures par une
// plateforme agréée. Beaucoup d'artisans ne l'ont pas fait, et personne ne les a
// prévenus. BatiSpot ne peut pas les inscrire à leur place — le raccordement en
// production demande un contrat — mais il peut faire les trois choses utiles :
// dire où ça se passe, retenir le choix, et ne plus jamais reposer la question.
//
// LES DEUX GRATUITES sont vérifiées chez elles le 13/09/2026 et présentes dans le
// fichier officiel impots.gouv.fr des opérateurs immatriculés. Leurs limites sont
// dites, pas cachées : Solo n'offre l'ÉMISSION que 30 jours, ce qui suffit pour
// 2026 (seule la réception est obligatoire) mais pas pour septembre 2027.
const PA_GRATUITES = [
  { nom: 'Tiime', quoi: 'émission et réception, gratuit sans limite de durée' },
  { nom: 'Solo', quoi: 'réception gratuite à vie ; émission incluse 30 jours seulement' },
];

async function plateformeFacturation(p) {
  const { data: proId } = await supabase.rpc('mon_entreprise');
  const { data: profil } = await supabase.from('pro_profiles')
    .select('pa_nom, pa_statut, pa_maj, siret').eq('pro_id', proId).single();

  const statutDemande = String(p.statut || '').trim().toLowerCase();
  const VALIDES = ['aucune', 'en_cours', 'active'];

  // ── Lecture seule : ce que BatiSpot sait déjà ────────────────────────────
  // L'assistant appelle ça d'abord. Sans ce garde-fou il réexpliquait la réforme
  // à un artisan déjà en règle — le pire moment pour paraître utile.
  if (!statutDemande) {
    const st = profil?.pa_statut || 'inconnue';
    const ou = profil?.pa_nom ? ` chez ${profil.pa_nom}` : '';
    const messages = {
      active: `Vous êtes en règle${ou} : vos factures fournisseurs arrivent bien par une plateforme agréée.`,
      en_cours: `Votre inscription${ou} était en cours. Elle est terminée ?`,
      aucune: 'Vous m\'avez dit ne pas encore avoir de plateforme agréée.',
      inconnue: 'Je ne sais pas encore où vous en êtes de la facture électronique.',
    };
    return {
      statut: st, plateforme: profil?.pa_nom || null, siret: profil?.siret || null,
      gratuites: PA_GRATUITES,
      message: messages[st],
      // Ce que l'assistant doit dire s'il doit expliquer — pas de prix, pas de
      // promesse, et l'interdiction de se présenter comme la plateforme.
      rappel: st === 'active' ? null
        : 'Depuis le 1er septembre 2026 toute entreprise doit pouvoir RECEVOIR ses factures '
          + 'par une plateforme agréée ; l\'émission devient obligatoire le 1er septembre 2027. '
          + 'Un PDF par mail ne compte plus. BatiSpot prépare la facture, la plateforme la transporte : '
          + 'ce sont deux choses différentes. Pour s\'inscrire il faut son SIREN et une pièce d\'identité '
          + 'du dirigeant. La liste officielle des plateformes est sur impots.gouv.fr — '
          + 'c\'est la seule qui fait foi.',
    };
  }

  if (!VALIDES.includes(statutDemande)) {
    throw new Error('Où en êtes-vous : pas de compte, inscription en cours, ou c\'est fait ?');
  }
  // « C'est fait » sans dire où : on ne peut pas router une facture vers un nom
  // qu'on n'a pas. On demande plutôt que d'enregistrer un statut creux.
  const nom = String(p.plateforme || '').trim().slice(0, 80) || profil?.pa_nom || null;
  if (statutDemande !== 'aucune' && !nom) {
    throw new Error('Chez quelle plateforme ? Il me faut le nom pour vos factures de l\'an prochain.');
  }

  const { error } = await supabase.from('pro_profiles').update({
    pa_statut: statutDemande,
    pa_nom: statutDemande === 'aucune' ? null : nom,
    pa_maj: new Date().toISOString(),
  }).eq('pro_id', proId);
  if (error) throw error;

  const dits = {
    aucune: 'C\'est noté. Deux plateformes agréées sont gratuites : '
      + PA_GRATUITES.map((g) => `${g.nom} (${g.quoi})`).join(', ')
      + '. L\'inscription prend dix minutes avec votre SIREN et votre pièce d\'identité. '
      + 'Je vous le rappellerai une fois, pas dix.',
    en_cours: `Inscription chez ${nom} notée comme en cours. Dites-moi quand c'est validé.`,
    active: `Parfait — ${nom} est enregistrée. Je ne vous en reparle plus.`,
  };
  return { statut: statutDemande, plateforme: statutDemande === 'aucune' ? null : nom, message: dits[statutDemande] };
}

// ---------------------------------------------------------------- toiture

// « Le toit de M. Vandaele à Roubaix, deux pans, 45 degrés » (13/09/2026,
// demande Moctar : « il faudra intégrer cette fonction dans l'assistant »).
//
// Un couvreur ne monte pas sur le toit pour le mesurer. L'emprise du bâtiment
// est publique (IGN), la pente est à lui. L'assistant POSE les questions qui
// manquent — l'adresse, la forme, la pente — et n'invente jamais la pente :
// c'est le seul endroit où le chiffre peut être faux, alors on le dit.
async function mesurerToiture(p) {
  const T = await import('./metre-toiture.js');

  let chantier = null;
  if (p.chantier_client) chantier = await trouverChantier(p.chantier_client);
  const adresse = String(p.adresse || (chantier && chantier.adresse) || '').trim();
  if (!adresse) {
    throw new Error(chantier
      ? `Le chantier de ${chantier.client_name} n'a pas d'adresse. Quelle est l'adresse du bâtiment ?`
      : 'Quelle est l\'adresse du bâtiment à couvrir ?');
  }

  const formes = { '2': '2_pans', '4': '4_pans', '1': '1_pan' };
  const forme = formes[String(p.forme || '').replace(/[^0-9]/g, '').charAt(0)] || null;
  if (!forme) {
    throw new Error('Le toit fait combien de pans — deux, quatre, ou un seul ?');
  }
  const pente = Number(String(p.pente_degres || '').replace(',', '.'));
  if (!(pente >= 5 && pente <= 70)) {
    throw new Error('Quelle est la pente du toit, en degrés ? Dans le Nord l\'ardoise se pose entre 40 et 45°, la tuile mécanique vers 35°. Je ne la devine pas : la surface en dépend.');
  }
  const materiau = String(p.materiau || 'ardoise').toLowerCase().replace(/[^a-z_]/g, '_');

  const point = await T.geocoder(adresse);
  if (!point) throw new Error(`Je ne trouve pas « ${adresse} » dans la base d'adresses. Vérifiez la rue et la commune.`);

  const bats = await T.batimentsAutour(point.lon, point.lat);
  if (!bats.length) {
    throw new Error(`Aucun bâtiment à « ${point.label} » dans la base de l'IGN — une construction très récente, peut-être. Donnez-moi l'emprise au sol en m² et la longueur du bâtiment, je calcule avec.`);
  }
  const bat = bats[0];
  const autres = bats.slice(1).filter((b) => b.aire >= 25 && b.distance < 40);

  // Le débord : demandé par l'assistant, 0,40 m si l'artisan dit « comme
  // d'habitude », 0 s'il dit « au nu du mur ». Jamais deviné en silence.
  const debord = Math.max(0, Math.min(1.5, Number(String(p.debord_m ?? '').replace(',', '.')) || 0));
  const t = T.calculerToiture({ aire: bat.aire, longueur: bat.longueur, largeur: bat.largeur, perimetre: bat.perimetre, forme, penteDeg: pente, materiau, debord });

  // Archivé comme les autres relevés : le client rappelle, les cotes sont là.
  let note = '';
  try {
    const { planSVG } = await import('./metre-plan.js');
    const { data: proId } = await supabase.rpc('mon_entreprise');
    const releve = { source: 'emprise', adresse: point.label, batiment: bat, toiture: t, points: bat.points,
                     surfaceSol: bat.aire, perimetre: bat.perimetre, hauteurSousPlafond: bat.hauteur, hauteurSupposee: true };
    const { error } = await supabase.from('releves_piece').insert({
      pro_id: proId, chantier_id: chantier ? chantier.id : null,
      piece: 'Toiture', methode: 'emprise',
      surface_sol: bat.aire, perimetre: bat.perimetre, hauteur: bat.hauteur, hauteur_supposee: true,
      donnees: releve, plan_svg: planSVG({ points: bat.points, piece: 'Toiture', surfaceSol: bat.aire }),
    });
    if (error) note = "\n\n(Je n'ai pas pu l'archiver, notez les cotes.)";
  } catch (_) { note = "\n\n(Je n'ai pas pu l'archiver, notez les cotes.)"; }

  const fr = (n, d = 1) => Number(n).toFixed(d).replace('.', ',');
  let message = `Toiture à ${point.label} — ${t.forme}, pente ${t.penteDeg}° : ${T.resumeParle(t)}.`
    + `\n\nEmprise IGN ${fr(bat.aire)} m² (${fr(bat.longueur)} × ${fr(bat.largeur)} m)` + (debord ? `, débord ${fr(debord, 2)} m compté` : ', sans débord')
    + (bat.hauteur ? `, hauteur ${fr(bat.hauteur)} m` : '') + `. ${t.formule}.`
    + `\n\n⚠️ À ${t.penteDeg - 5}° au lieu de ${t.penteDeg}°, la surface ferait ${fr(t.surfaceSiMoins5)} m² : vérifiez la pente avant de chiffrer.`;
  if (autres.length) {
    message += `\n\nIl y a aussi ${autres.length === 1 ? 'un bâtiment' : autres.length + ' bâtiments'} à côté (`
      + autres.map((b) => `${fr(b.aire)} m²`).join(', ') + `) — dites-moi si vous les couvrez aussi.`;
  }
  message += (chantier ? `\n\nRangé dans le dossier de ${chantier.client_name}.` : '') + '\n\nQuels travaux sur cette toiture ?' + note;

  // Le détail du calcul voyage avec le résultat : si le couvreur demande
  // « comment tu trouves ça ? », le modèle a les formules ET les nombres.
  return { message, releve: t, explication: T.expliquer(bat, t), lien: chantier ? `./chantier.html?id=${chantier.id}` : undefined };
}
...
const ACTIONS = {
  ...
  marquer_devis_refuse: marquerDevisRefuse,
  mesurer_toiture: mesurerToiture,
  plateforme_facturation: plateformeFacturation,
