...
// supabase-js est servi depuis NOTRE domaine (app/js/vendor/supabase-js.js),
// pas depuis esm.sh : ce CDN imposait 5 allers-retours reseau en cascade
// avant le moindre appel, soit 2-3 s d'ecran vide en 4G. Voir l'en-tete du
// fichier vendor. NE PAS reintroduire d'import CDN ici.
import { createClient } from './vendor/supabase-js.js';
// Heures d'intervention : un seul endroit qui sait lire « 9h30 » (js/heures.js).
import { normaliserHeure } from './heures.js';
export { normaliserHeure, heureLisible, heureFin, creneauLisible } from './heures.js';
...
// ── Appareil reconnu du client (25/09/2026) ──────────────────────────────────
// Le suivi de chantier s'ouvre avec un lien public. Avant, ce lien suffisait :
// qui l'avait voyait les devis, les factures et les messages. Desormais le
// client valide un code a 6 chiffres la premiere fois, et son appareil recoit
// un jeton qu'il presente a chaque requete. Le serveur refuse sans lui.
//
// Pose ici pour la meme raison que le delai reseau : un seul endroit couvre
// tous les appels, `rpc` et `functions.invoke` compris.
let jetonAppareilClient = '';
// Un jeton d'appareil est un UUID, donc de l'ASCII pur. On le verifie quand
// meme : un en-tete HTTP contenant un caractere accentue fait echouer le
// parsing des en-tetes cote PostgREST et rend une erreur 500 (verifie le
// 25/09). L'acces reste refuse, mais autant ne pas provoquer la panne.
export function poserAppareilClient(jeton) {
  jetonAppareilClient = /^[\x21-\x7e]*$/.test(jeton || '') ? (jeton || '') : '';
}

// Ajoute l'en-tete d'appareil client sans perdre ce qui etait deja pose.
//
// 25/09/2026 — le piege qui a casse la page de suivi : supabase-js ne passe pas
// toujours `headers` comme objet simple, il passe parfois un objet `Headers`.
// Etaler un `Headers` avec `{ ...h }` ne rend pas ses entrees mais un objet
// VIDE : `apikey` et `Authorization` disparaissaient, et la requete repartait
// en 401. Le client voyait sa porte s'ouvrir puis une page vide. On normalise
// donc les trois formes que la specification autorise avant d'ajouter la notre.
function avecEnteteAppareil(options) {
  if (!jetonAppareilClient) return options;
  const h = options.headers;
  let fusion;
  if (typeof Headers !== 'undefined' && h instanceof Headers) {
    fusion = new Headers(h);
    fusion.set('x-bs-appareil', jetonAppareilClient);
  } else if (Array.isArray(h)) {
    fusion = h.filter((p) => String(p?.[0]).toLowerCase() !== 'x-bs-appareil')
              .concat([['x-bs-appareil', jetonAppareilClient]]);
  } else {
    fusion = { ...(h || {}), 'x-bs-appareil': jetonAppareilClient };
  }
  return { ...options, headers: fusion };
}

function fetchAvecDelai(url, options = {}) {
  options = avecEnteteAppareil(options);
...
// ── Démonstration de la visite guidée (25/09/2026) ───────────────────────────
// Un artisan qui vient de s'inscrire arrive sur des écrans vides : la visite
// lui montrait où appuyer devant du néant. On pose trois chantiers, quatre
// devis, deux encaissements et des dépenses le temps de la visite, puis on
// retire tout — « pour tout vrai artisan la plateforme doit être vide »
// (Moctar, 25/09). Rien n'est posé sur un compte qui a déjà de vrais chantiers.
//
// Exposées sur `window` parce que app-wizard.js est un script classique, pas un
// module : il ne peut pas les importer.
export async function poserDemoVisite() {
  const { data, error } = await supabase.rpc('poser_demo_visite');
  if (error) throw error;
  return data || {};
}
export async function retirerDemoVisite() {
  const { data, error } = await supabase.rpc('retirer_demo_visite');
  if (error) throw error;
  return data || {};
}
try {
  window.bsDemoVisite = { poser: poserDemoVisite, retirer: retirerDemoVisite };
} catch (_) { /* pas de window : test en Node */ }
...
export async function listMessages(chantierId) {
  // Sans identifiant, PostgREST reçoit `chantier_id=eq.undefined` et répond 400
  // « invalid input syntax for type uuid » : un échec technique là où il n'y a
  // simplement rien à montrer. On le dit clairement au lieu de le subir.
  if (!chantierId) throw new Error('Chantier inconnu : impossible de charger le fil.');
  // LES 200 DERNIERS, pas tout le fil. Un `select *` sans borne grandit avec le
  // chantier, et le fetch de l'application abandonne à 15 secondes
  // (fetchAvecDelai) : passé un certain volume, le fil devient « indisponible »
  // sans que rien ne soit cassé. On prend les plus RÉCENTS — donc tri
  // décroissant en base — puis on les remet dans l'ordre de lecture.
  const { data, error } = await supabase
    .from('messages')
    .select('*')
    .eq('chantier_id', chantierId)
...
// ── Le portier du lien de suivi (25/09/2026) ─────────────────────────────────
// Tant que l'appareil n'est pas reconnu, les fonctions ci-dessous sont les
// seules que le client peut appeler. Voir supabase/functions/acces-client.

/** Demande l'envoi d'un code a 6 chiffres. Rend { protege, envoye, email }
 *  ou `email` est masquee (« m•••••e@gmail.com ») : le client reconnait la
 *  sienne, un tiers n'apprend rien. `protege: false` = ce chantier n'a pas
 *  d'adresse client enregistree, l'acces reste celui d'avant. */
export async function pub_demanderCode(token) {
  const { data, error } = await supabase.functions.invoke('acces-client', {
    body: { action: 'demander', token },
  });
  if (error) throw error;
  return data || {};
}

/** Verifie le code saisi. Si bon, rend { valide: true, appareil } — le jeton
 *  a garder pour ne plus redemander. Sinon { valide: false, motif } ou motif
 *  vaut 'faux', 'expire', 'trop_essais' ou 'format'. */
export async function pub_verifierCode(token, code, etiquette) {
  const { data, error } = await supabase.functions.invoke('acces-client', {
    body: { action: 'verifier', token, code, etiquette },
  });
  if (error) throw error;
  return data || {};
}
...
// Le client corrige ou efface SON message (13/09/2026, « comme WhatsApp »,
// même règle que l'artisan) : tant que l'artisan ne l'a pas lu, et dans les
// 15 minutes. La garde est dans les fonctions serveur (edit_client_message /
// delete_client_message) — ici on appelle, et on montre le refus tel quel.
export async function pub_editMessage(token, id, content) {
  const { error } = await supabase.rpc('edit_client_message', { p_token: token, p_id: id, p_content: content });
  if (error) throw error;
}
export async function pub_deleteMessage(token, id) {
  const { error } = await supabase.rpc('delete_client_message', { p_token: token, p_id: id });
  if (error) throw error;
}
...
export async function resolveUserRole() {
  ...
  const userId = session.user.id;
  const email = (session.user.email || '').toLowerCase();
  // 22/09/2026 : une requete en ERREUR (reseau coupe 2 s, 429, serveur
  // injoignable) rendait `data: null`, lu comme « pas artisan » — et l'artisan
  // atterrissait dans l'espace particulier, sans porte de sortie. C'est arrive
  // a Moctar le 22/09 au matin. On distingue desormais « la base dit non »
  // (toutes les requetes ont repondu, aucune ligne) de « on n'a pas pu
  // demander » (au moins une erreur) : dans ce cas on rend null et l'appelant
  // tranche — l'appli est celle des artisans, il envoie au dashboard.
  let erreur = false;
  const { data: pro, error: ePro } = await supabase
  ...
  let { data: lead, error: eLead } = await supabase
    .from('artisan_leads').select('id').eq('user_id', userId).limit(1).maybeSingle();
  if (eLead) erreur = true;
  if (!lead && email) {
    ({ data: lead, error: eLead } = await supabase
  ...
  if (erreur) {
    console.warn('[BatiSpot] Role non verifiable (requete en erreur) — l\'appelant tranche.');
    return null;
  }
  return 'client';
}
...
// ── Main courante de chantier (07/09/2026) ───────────────────
// Ce qui a été constaté, daté, avec photo. Sert de preuve pour les travaux
// supplémentaires (art. 1793), les réserves et les désordres découverts.
// Un constat n'est pas un message : il survit à la suppression d'un fil, il
// porte une date qui fait foi, et il part dans le dossier remis au client.

export async function listConstats(chantierId, limite = 50) {
  const { data, error } = await supabase
    .from('constats_chantier')
    .select('id, type, texte, photo_id, constate_le, client_informe_le, photos(storage_path, caption)')
    .eq('chantier_id', chantierId)
...
export async function creerConstat({ chantier_id, type = 'note', texte, photo_id = null, constate_le = null }) {
  const session = await getSession();
  const d = await mesDroits();
  const { data, error } = await supabase.from('constats_chantier')
    .insert({
      chantier_id, pro_id: d.entreprise, type, texte: String(texte || '').trim(),
      photo_id, auteur_id: session ? session.user.id : null,
...
// Un constat que le client ignore ne protège de rien : c'est l'information qui
// fait la preuve, pas la note dans un coin.
export async function marquerConstatInforme(id) {
  const { error } = await supabase.from('constats_chantier')
    .update({ client_informe_le: new Date().toISOString() }).eq('id', id);
  if (error) throw error;
}

// ── Indisponibilités (07/09/2026) ────────────────────────────
// « Bloque mon vendredi après-midi, je prends mon RTT. » Une absence n'est pas
// un chantier : elle a sa table, avec les mêmes règles d'accès que le reste
// (chacun chez soi, l'équipe voit les absences de l'équipe — on ne planifie
// pas quelqu'un qui est en congé).

export async function listIndisponibilites(debut, fin) {
  let q = supabase.from('indisponibilites')
    .select('id, jour, jour_fin, moment, motif, qui')
    .order('jour', { ascending: true });
  if (debut) q = q.gte('jour', debut);
  if (fin) q = q.lte('jour', fin);
  const { data, error } = await q;
  if (error) throw error;
  return data || [];
}

export async function creerIndisponibilite({ jour, jour_fin = null, moment = 'journee', motif = null, qui = null }) {
  const d = await mesDroits();
  const { data, error } = await supabase.from('indisponibilites')
    .insert({ pro_id: d.entreprise, jour, jour_fin, moment, motif, qui })
    .select().single();
  if (error) throw error;
  return data;
}

export async function supprimerIndisponibilite(id) {
...
// ── Groupes d'équipe LIBRES (12/09/2026, Moctar : « je dois pouvoir créer des
// groupes sans lien avec le chantier ») — table groupes_equipe, messages.groupe_id.
export async function listGroupesLibres() {
  const { data, error } = await supabase
    .from('groupes_equipe').select('id, nom, membres, created_at')
    .order('created_at', { ascending: false });
  if (error) throw error;
  return data || [];
}
export async function creerGroupeLibre(nom, membresIds) {
  const { data: proId, error: e0 } = await supabase.rpc('mon_entreprise');
  if (e0) throw e0;
  const { data, error } = await supabase
    .from('groupes_equipe')
    .insert({ pro_id: proId, nom: String(nom || '').trim(), membres: (membresIds || []).filter(Boolean) })
    .select().single();
  if (error) throw error;
  return data;
}
export async function listMessagesGroupe(groupeId) {
  const { data, error } = await supabase
    .from('messages').select('*')
    .eq('groupe_id', groupeId).eq('visible_client', false)
    .order('created_at', { ascending: true });
  if (error) throw error;
  return data;
}
export async function envoyerMessageGroupe(groupeId, contenu) {
  const session = await getSession();
  const moi = await monProfilEquipe();
  const { data, error } = await supabase
    .from('messages')
    .insert({ groupe_id: groupeId, author: moi ? 'membre' : 'pro', auteur_id: session.user.id, content: contenu, visible_client: false })
    .select().single();
  if (error) throw error;
  return data;
}
...
export function suivreMessagesGroupe(groupeId, onMessage) {
  return supabase
    .channel(`groupe:${groupeId}`)
    .on('postgres_changes',
      { event: 'INSERT', schema: 'public', table: 'messages', filter: `groupe_id=eq.${groupeId}` },
      (payload) => { if (payload.new && payload.new.visible_client === false) onMessage(payload.new); })
    .subscribe();
}
export async function marquerLuMessagesGroupe(groupeId, monId) {
  try {
    await supabase.from('messages').update({ lu_le: new Date().toISOString() })
      .eq('groupe_id', groupeId).is('lu_le', null).neq('auteur_id', monId);
  } catch (e) { console.warn('[groupe] marquer lu', e); }
}
...
export async function listTachesChantier(chantierId) {
  const { data, error } = await supabase
    ...
    // « 8 h dépannage Benali » puis les étapes de fond de la journée.
...
export async function listTachesJour(jour) {
  ...
  const { data, error } = await supabase
    .from('taches').select('*')
    .lte('jour', jour)
    // (12/09) une tâche SANS date de fin n'est « du jour » que si elle est ce
    // jour-là ; avant, toute tâche passée non terminée réapparaissait chaque jour.
    .or(`jour_fin.gte.${jour},and(jour_fin.is.null,jour.eq.${jour})`)
    .order('heure_debut', { ascending: true, nullsFirst: false })
    .order('ordre', { ascending: true });
  if (error) throw error;
...
export async function creerTache(chantierId, champs = {}) {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      // Heure de début (07/09) : facultative par nature. Vide = étape à la
      // journée, comme depuis toujours ; renseignée = créneau, pour les
      // métiers qui font plusieurs interventions dans la même journée.
      heure_debut: normaliserHeure(champs.heure_debut ?? champs.heure),
...
export async function modifierTache(id, champs = {}) {
  ...
  // `heure_debut: null` est une demande explicite (« remets-la à la journée »),
  // pas une absence : on la distingue de `undefined`, qui veut dire « je n'y
  // touche pas ». `heure` est accepté comme synonyme — c'est le mot que
  // l'assistant emploie.
  const heure = champs.heure_debut !== undefined ? champs.heure_debut : champs.heure;
  if (heure !== undefined) patch.heure_debut = normaliserHeure(heure);
  const { data, error } = await supabase
    .from('taches')
    .update(patch)
    .eq('id', id).select().single();
...
/**
 * Range au COFFRE un justificatif deja depose par `uploadJustificatif`.
 *
 * Pourquoi une fonction separee de `uploadCoffreDoc` : celle-ci televerse son
 * propre fichier, alors qu'ici la photo est deja dans le bucket. On ne fait donc
 * que poser la ligne, en la reliant a la depense qu'elle justifie.
 *
 * C'est ce qui donne le classeur que Moctar appelle « comme CamScanner »
 * (09/09/2026) : le rangement par date et par type n'est pas un tri a faire,
 * c'est une consequence de `created_at`, `categorie` et `chantier_id`. Le NOM
 * porte le fournisseur et la date, pour que la liste se lise sans l'ouvrir.
 *
 * Ne jette jamais : un echec ici ne doit pas faire perdre la depense, l'appelant
 * traite l'erreur et continue.
 */
export async function lierJustificatifAuCoffre({
  chemin, depense_id = null, chantier_id = null,
  fournisseur = null, date_achat = null, categorie = 'autre',
  type_mime = 'image/jpeg', taille_octets = null,
}) {
  const session = await getSession();
  const jour = date_achat
    ? new Date(date_achat).toLocaleDateString('fr-FR')
    : new Date().toLocaleDateString('fr-FR');
  const nom = `Justificatif ${fournisseur || 'sans fournisseur'} — ${jour}`.slice(0, 180);
  const { data, error } = await supabase
    .from('coffre_documents')
    .insert({
      pro_id: session.user.id,
      nom,
      categorie: (categorie || 'autre').toLowerCase(),
      chemin,
      type_mime,
      taille_octets,
      chantier_id,
...
export async function coffreDocsExpirants(joursAvant = 60) {
  const limite = new Date(Date.now() + joursAvant * 86400000 - new Date().getTimezoneOffset() * 60000).toISOString().slice(0, 10);
  const { data, error } = await supabase
    .from('coffre_documents')
