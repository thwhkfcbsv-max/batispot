 ...
 */
// ── Recensement des appareils (19/09/2026) ────────────────────────────────
// Moctar : « je veux qu'on journalise son résultat ». Depuis qu'on a retiré le
// repli photo, un téléphone sans AR ne peut plus rien mesurer — il faut donc
// savoir QUELLE PROPORTION de nos artisans est concernée, sur nos artisans et
// pas sur une statistique de marché. Une ligne par artisan et par jour
// (`capacites_appareil`), aucune donnée perso : la méthode et le modèle.
// Fail-silent et jamais attendu : ceci ne doit jamais retarder l'écran.
let _journalise = false;
function journaliserCapacite(cap) {
  try {
    if (_journalise) return;
    const cfg = globalThis.window?.__BATISPOT_CONFIG__ || {};
    const jeton = typeof globalThis.window?.bsJetonSession === 'function' ? globalThis.window.bsJetonSession() : null;
    // Sans session verifiee la RPC refuse : on sort SANS poser le drapeau, pour
    // que l'appel suivant (session restauree entre-temps) compte l'appareil.
    // Poser le drapeau avant l'envoi perdait silencieusement les artisans dont
    // la session arrive apres le premier passage.
    if (!jeton || !cfg.SUPABASE_ANON_KEY) return;
    _journalise = true;
    fetch((cfg.SUPABASE_URL || 'https://cisniwhaiydazdpzvino.supabase.co') + '/rest/v1/rpc/capacite_appareil_journaliser', {
      method: 'POST',
      headers: { 'content-type': 'application/json', apikey: cfg.SUPABASE_ANON_KEY, authorization: 'Bearer ' + jeton },
      body: JSON.stringify({
        p_methode: cap.methode === 'points' || cap.methode === 'balayage3d' ? cap.methode : 'aucune',
        p_natif: !!cap.natif,
        p_webxr: !!cap.webxr,
        p_ua: String(globalThis.navigator?.userAgent || '').slice(0, 300),
      }),
      keepalive: true,
...
export async function capacitesMetre() {
  ...
  if (p) {
    try {
      ...
      journaliserCapacite(capNative);
      return capNative;
    } catch (_) { /* on retombe sur le web */ }
  }
  // (19/09/2026) Détection WebXR rétablie. `app/metre.html` est revenu le 16/09
  // avec son viseur complet ; cette fonction répondait encore « photo » en dur
  // (note du 13/09, quand l'écran avait disparu) — l'app ne proposait donc
  // jamais le métré à un Android compatible. Décision Moctar 19/09 : plus de
  // repli « photo ». Soit l'appareil mesure (points au sol, Chrome + ARCore),
  // soit on le dit (« aucune ») et Studio garde le ticket et les photos.
  // `natif: false` → app-actions.js ouvre metre.html au lieu d'appeler le plugin.
  try {
    if (globalThis.navigator?.xr && await navigator.xr.isSessionSupported('immersive-ar')) {
      const capWeb = { methode: 'points', balayage3d: false, natif: false, webxr: true, libelle: 'Metre par points au sol' };
      journaliserCapacite(capWeb);
      return capWeb;
    }
  } catch (_) { /* pas de WebXR : on tombe sur « aucune » */ }
  const capAucune = { methode: 'aucune', balayage3d: false, natif: false, webxr: false, libelle: 'Non disponible sur cet appareil' };
  journaliserCapacite(capAucune);
  return capAucune;
}
