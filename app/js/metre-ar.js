...
export async function capacitesMetre() {
  ...
  // ⚠️ 13/09/2026 — le chemin WebXR ne mene plus nulle part.
  //
  // Jusqu'au 04/09 un Android sous Chrome relevait la piece en WebXR/ARCore
  // depuis `app/metre.html`. Ce fichier a ete supprime par le commit
  // df8b7027e (« Replique de la maquette validee sur toute l'application ») :
  // l'ecran qui portait le viseur n'existe plus, et `releverPiece()` ci-dessous
  // exige desormais le plugin natif.
  //
  // Or cette fonction continuait a repondre « points » des que navigator.xr
  // disait oui. Consequence sur un Android avec ARCore : app-actions.js voyait
  // une methode disponible, ne servait donc PAS le repli photo, appelait
  // releverPiece() — qui jetait « Le metre AR demande l'application installee ».
  // L'artisan recevait une erreur brute au lieu d'une explication. C'est
  // exactement ce que les testeurs Android du test ferme Play auraient vu.
  //
  // Tant qu'aucun ecran ne consomme WebXR, on dit la verite : sur le web, la
  // mesure se fait par photo. Le jour ou le viseur revient, il suffit de
  // retablir la detection ici — la geometrie (metre-geometrie.js) et le plan
  // cote (metre-plan.js) n'ont jamais bouge.
  return { methode: 'photo', balayage3d: false, libelle: 'Mesure par photo avec une feuille A4' };
}
