...
// Transcription rendue mais peu sûre : l'artisan la voit et la relit.
export const MESSAGE_DOUTE = "Je ne suis pas sûr d'avoir tout compris — relisez avant de valider.";
...
export async function transcrire({ blob, mime, dureeS, niveauDb, creteDb, dureeUtileS }, { contexte = 'devis', texteAvant = '' } = {}) {
  ...
  return {
    ...
    // (25/09) Le moteur dit lui-même quand il n'est pas sûr. Mesuré sur
    // 40 dictées : sa confiance prédit son erreur réelle à -0,85 de
    // corrélation. En dessous du seuil de doute, le texte est rendu quand
    // même — un artisan qui parle français avec un accent doit obtenir sa
    // transcription — mais l'écran doit dire de la relire avant de valider.
    doute: !vide && j.doute === true,
    confiance: typeof j.confiance === 'number' ? j.confiance : null,
  };
}
 ...
 */
export function brancherBoutonMaintenu(bouton, { contexte = 'devis', texteAvant = () => '', onDebut, onAttente, onFin, onErreur, onInfo, onVerrou } = {}) {
  let enr = null;
  ...
  // ── APPUI SIMPLE, à la WhatsApp (Moctar, 09/09) ────────────────────────
  // « il faut rester appuyé, il vaut mieux un système à la WhatsApp : t'appuies
  // ça enregistre, et quand tu réappuies tu peux envoyer le message. »
  // Tenir le bouton une main gantée, sur un chantier, en lisant l'écran, ce
  // n'est pas tenable. Donc : un appui COURT verrouille l'enregistrement, qui
  // continue tout seul, et le prochain appui l'arrête. Un appui LONG garde
  // exactement le comportement d'avant — on n'enlève rien à qui l'a appris.
  const SEUIL_APPUI_COURT = 400;  // ms
  ...
  const MSG_AUTORISE = 'Micro autorisé · touchez le bouton pour dicter';
  let tRelache = 0;
  let permisOk = false;
  ...
  const debut = async (e) => {
    ...
    e.preventDefault();
    // Enregistrement verrouillé : cet appui-ci l'ARRÊTE. `fin` voit `verrouille`
    // levé, saute la fenêtre d'appui court, et transcrit normalement.
    if (verrouille && enr && enr.actif) { await fin(e); return; }
    tAppui = Date.now();
    ...
    relache = false;
    demarrage = (async () => {
      // (12/09, Moctar : « et le micro c'est résolu ? » — zéro trace en 40 h.)
      // Sur iPhone, Safari REPOSE la boîte « Autoriser le micro ? » à chaque
      // session : le doigt se relâche pendant la boîte, et l'ancien code
      // annulait tout en affichant « maintenez le bouton » — à chaque essai,
      // sans jamais rien enregistrer ni rien journaliser. Désormais un appui
      // COURT relâché pendant l'ouverture du micro VERROUILLE l'enregistrement
      // dès que le micro tourne (comme un appui court normal) ; seul un appui
      // long relâché avant l'ouverture est annulé.
      const appuiCourt = () => (tRelache - tAppui) <= SEUIL_APPUI_COURT;
      if (!(await permissionConnue())) { if (!(await demanderSeule())) return false; }
      if (relache && !appuiCourt()) return false;
      enr = new Enregistreur();
      await enr.demarrer();
      noterPermis();
      if (relache) {
        if (appuiCourt()) { verrouille = true; onVerrou && onVerrou(); return true; }
        // Appui long relâché avant que le micro tourne : on ne laisse pas
        // tourner un micro que personne ne tient.
  ...
  const fin = async (e) => {
    // Appui court : on ne coupe pas, on VERROUILLE. L'enregistrement continue
    // sans le doigt ; le prochain appui appellera `fin` avec `verrouille` levé.
    if (!verrouille && enr && enr.actif && (Date.now() - tAppui) < SEUIL_APPUI_COURT) {
    ...
    verrouille = false;
    if (demarrage) { relache = true; tRelache = Date.now(); return; }
    ...
    try {
      ...
      onFin && onFin(
        res.vide ? { ...res, texte: '', incertain: [], message: MESSAGE_RIEN_ENTENDU }
        : res.doute ? { ...res, message: MESSAGE_DOUTE }
        : res);
    } catch (err) {
      onErreur && onErreur(messageErreurDictee(err));
    } finally {
  ...
  // Une fois verrouillé, le doigt n'a plus rien à voir avec l'enregistrement :
  // un pointercancel (notification, rotation, boîte système) ne doit plus le
  // couper. Sans ce garde, le verrouillage sauterait au premier aléa.
  const finSiTenu = (e) => { if (!verrouille) fin(e); };
  bouton.addEventListener('pointercancel', finSiTenu);
  bouton.addEventListener('lostpointercapture', finSiTenu);
  ...
  return () => {
    ...
    bouton.removeEventListener('pointercancel', finSiTenu);
    bouton.removeEventListener('lostpointercapture', finSiTenu);
