...
/** `No session` : sept ecrans (client, coffre, documents, chantier…) levent cette
 *  erreur APRES que requireAuth() a deja redirige vers la connexion — c'est un
 *  arret volontaire du module, pas une panne. Journalise le 22/09/2026 comme
 *  « plantage » (3 occurrences, en fait un test automatise). Exclu ici plutot
 *  que reecrit page par page : le throw reste le moyen le plus sur d'arreter un
 *  module ESM en plein vol. */
const BRUIT = /ResizeObserver loop|^Script error\.?$|Non-Error promise rejection|Load failed|NetworkError when attempting|^(Uncaught )?Error: No session$|^No session$/i;
