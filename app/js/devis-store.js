...
export function rangeeVersQuote(row) {
  ...
  return {
    ...
    // (24/09) Les dates d'ENVOI et de SIGNATURE existent en base : « signés ce mois »
    // se comptait sur la création et rangeait en septembre des devis signés en août.
...
async function pousserEnBase(entree) {
  ...
  // Un brouillon garde sa référence provisoire : le trigger en base lui
  // donnera son numéro le jour où il sortira du brouillon. Un devis déjà
  // porteur d'un numéro de série (ou créé directement en envoyé) passe par
  // numeroLibre comme avant.
  const numero = (statutPourBase(q) === 'brouillon' && estRefProvisoire(q.id))
    ? q.id
    : await numeroLibre(q.id);
  ...
  // Le devis est en base : c'est le signal d'activation. Un compte sans devis
  // n'a rien prouve. Emis ici et pas a l'enregistrement local, qui peut ne
  // jamais atteindre la base.
  try {
    const { devisCree } = await import('./conversion.js');
    devisCree(base?.metier || null);
  } catch (_) {}
  return (await relireDevis(data.id)) || data;
}
...
export async function enregistrerDevis(quote) {
  ...
  // Un brouillon prend une référence provisoire ; seul un devis qui part
  // vraiment chez le client consomme un numéro de la série.
  if (!q.id) q.id = estBrouillonDevis(q) ? refProvisoire() : numeroLocalSuivant();
...
// ── Le brouillon ne consomme pas de numéro (13/09/2026) ───────────────────
// Un brouillon n'a pas été remis au client : ce n'est ni une offre ni un
// contrat, il n'a aucune existence juridique. Il ne doit donc PAS prendre un
// numéro de la série. Avant ce correctif, les 6 brouillons en base portaient
// déjà un `DEV-2026-…` : en supprimer un faisait un trou dans la série, et
// comme `numeroLibre` calcule max+1, le devis suivant reprenait exactement le
// même numéro que celui qu'on venait de jeter.
//
// Il porte donc une référence interne `BR-XXXXXXXX`. Elle reste UNIQUE, ce qui
// laisse l'index (pro_id, numero) du 05/09 continuer de bloquer les doublons
// de synchronisation — c'était son autre rôle. Le trigger
// `devis_numero_emission` la remplace par le vrai numéro au moment où le devis
// sort du brouillon, et ce numéro ne bouge plus jamais ensuite.
function refProvisoire() {
  return 'BR-' + uuid().replace(/-/g, '').slice(0, 8).toUpperCase();
}

export function estRefProvisoire(numero) {
  return /^BR-/i.test(String(numero || ''));
}

// Ce qu'on montre à l'artisan : un brouillon n'a pas de numéro, il a un état.
export function referenceLisible(quote) {
  if (!quote) return '';
  return estRefProvisoire(quote.id) ? 'Brouillon' : String(quote.id || '');
}

/**
 * Supprime un BROUILLON — et rien d'autre.
 *
 * Librement effaçable : rien n'a été remis au client, aucun numéro de série
 * n'a été consommé. Tout le reste se conserve. Un devis ENVOYÉ est une offre
 * ferme qui engage l'artisan pendant sa durée de validité ; un devis ACCEPTÉ
 * est un contrat signé, et la pièce justificative de la facture qui suit.
 *
 * Le vrai verrou est en base : la policy `devis_delete_encadrement`
 * (13/09/2026) exige `status = 'brouillon'`, donc même une requête forgée à la
 * main avec le jeton de l'artisan ne peut pas effacer un devis émis. Le garde
 * ci-dessous évite seulement un aller-retour inutile et donne un message clair.
 */
export async function supprimerBrouillon(quote) {
  if (!quote || !quote.id) throw new Error('Devis introuvable.');
  if (!estBrouillonDevis(quote)) {
    throw new Error("Seul un brouillon peut être supprimé. Un devis envoyé ou accepté se conserve.");
  }

  // La base d'abord. Si elle refuse, le devis reste à l'écran et l'artisan
  // peut réessayer — l'inverse le ferait disparaître de l'écran sans l'avoir
  // effacé, et il réapparaîtrait à la prochaine synchronisation.
  const entrees = lireLocal();
  const locale = entrees.find((e) => e.quote && e.quote.id === quote.id);
  const dbId = quote._dbId || (locale && locale.dbId) || null;

  const session = await getSession();
  if (session && (dbId || quote.id)) {
    const requete = supabase.from(TABLE_DEVIS).delete();
    const { error } = dbId
      ? await requete.eq('id', dbId)
      : await requete.eq('numero', quote.id).eq('status', 'brouillon');
    // 42501 / ligne absente : rien à faire de plus, le brouillon n'est pas (ou
    // plus) en base. On nettoie quand même le journal local.
    if (error && error.code !== 'PGRST116') throw error;
  }

  ecrireLocal(entrees.filter((e) => !(e.quote && e.quote.id === quote.id)));
  return true;
}
