...
// (A68, audit 12/09 — Moctar : « les documents comme le PV sont bien au nom
// de l'entreprise ? ») Le PV imprimé ne nommait PAS l'entreprise : seulement
// « L'entreprise — date et signature ». La fiche est lue une fois, à l'ouverture,
// pour que l'impression reste synchrone (window.open dans le clic, sinon iOS
// la bloque).
let profilEntreprise = null;
try { getMyProfile().then((p) => { profilEntreprise = p || null; }).catch(() => {}); } catch (_) {}
...
function ligneDocument({ titre, detail, etat, action }) {
  ...
  if (detail) mid.appendChild(el('div', 'font-size:12.5px;color:#5A7268;margin-top:2px;line-height:1.4', detail));
...
function boutonPetit(texte, onClick, primaire) {
  const b = el('button', `padding:6px 10px;font-size:12.5px;font-weight:800;border-radius:8px;cursor:pointer;white-space:nowrap;flex-shrink:0;border:1.5px solid #228B5B;${primaire ? 'background:#228B5B;color:#fff' : 'background:#fff;color:#0F5132'}`, texte);
  b.type = 'button';
...
export async function renderDossierChantier(conteneur, chantier) {
  ...
  listeAvant.appendChild(el('p', 'font-size:12.5px;color:#5A7268;margin:2px 0 0;line-height:1.4',
  ...
  if (chantier.status === 'termine' || toutesEtapesCochees) {
    ...
    if (pv) {
      ...
      if (!pv.signe_le) {
        listeFin.appendChild(el('div', 'font-size:12.5px;color:#5A7268;padding:0 0 4px 30px;line-height:1.45',
    ...
    if (!pv) {
      carteFin.appendChild(el('p', 'font-size:12.5px;color:#5A7268;margin:0;line-height:1.45',
...
function ouvrirFormulairePv(conteneur, chantier, pvExistant) {
  ...
  iDate.value = (pvExistant && pvExistant.date_reception) || new Date(Date.now() - new Date().getTimezoneOffset() * 60000).toISOString().slice(0, 10);
...
function ouvrirSignaturePv(conteneur, chantier, pv) {
  ...
  carte.appendChild(el('div', 'font-size:12.5px;color:#5A7268;margin-top:2px', 'Signature, au doigt :'));
  ...
  carte.appendChild(el('p', 'font-size:12.5px;color:#5A7268;margin:0;line-height:1.4',
...
<style>
  ...
  .sign strong { display:block; font-size:12.5px; margin-bottom:4px; }
  .pied { margin-top:22px; font-size:12.5px; color:#5A7268; line-height:1.5; }
...
<h2>Chantier</h2>
<table class="champs">
  <tr><td>Entreprise</td><td><strong>${esc((profilEntreprise && profilEntreprise.display_name) || '—')}</strong>${profilEntreprise && profilEntreprise.siret ? ' · SIRET ' + esc(profilEntreprise.siret) : ''}${profilEntreprise && profilEntreprise.address ? '<br>' + esc(profilEntreprise.address) : ''}</td></tr>
...
<div class="sign">
  <div><strong>Le maître d’ouvrage</strong>${pv.signataire_nom ? '<br>' + esc(pv.signataire_nom) : ''}
    <br><span style="color:#5A7268;font-size:12.5px">Date et signature, précédées de « Reçu »</span></div>
  <div><strong>L’entreprise</strong>${profilEntreprise && profilEntreprise.display_name ? '<br>' + esc(profilEntreprise.display_name) : ''}<br><span style="color:#5A7268;font-size:12.5px">Date et signature</span></div>
</div>
