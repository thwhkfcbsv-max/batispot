...
export async function rendreVolet(conteneur, quote, opts = {}) {
  ...
  const dessiner = () => {
    ...
    liste.forEach((m) => {
      ...
      if (m.base) {
        const b = document.createElement('span'); b.style.cssText = 'font-size:12.5px;color:#5A7268;'; b.textContent = m.base; carte.appendChild(b);
      }
      ...
      lien.type = 'button';
      lien.style.cssText = 'background:none;border:none;padding:0;margin-top:2px;align-self:flex-start;font-size:12.5px;font-weight:800;color:#1B7049;cursor:pointer;text-decoration:underline;';
...
function dessinerOutillage(conteneur, quote, cache, opts) {
  ...
  cache.outillage.forEach((o) => {
    ...
    if (o.pourLigne) {
      ...
      l.style.cssText = 'margin-left:auto;color:#8AA096;font-size:12.5px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:40%;';
      l.title = o.pourLigne;
  ...
  note.style.cssText = 'font-size:12.5px;color:#5A7268;padding:6px 2px 0;line-height:1.45;';
