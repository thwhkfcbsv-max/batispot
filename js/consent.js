...
(function () {
  ...
  function injectStyles() {
    ...
    s.textContent = [
      ...
      'background:#fff;border:1.5px solid #228B5B;border-radius:14px;',
      ...
      'body.bs-consent-open .modal-overlay,'
        ...
        + 'body.bs-consent-open [role="dialog"]:not(#bs-consent){padding-bottom:var(--bs-consent-h,180px)!important}',
            // 44 px de haut : ce sont les DEUX PREMIERS boutons que touche un
      // visiteur, et 10px de padding sur du 13px donnait 38 px. Attrape par
      // le test de robustesse le 08/09, qui a bloque la publication.
      '#bs-consent button{flex:1;min-width:120px;padding:13px 14px;min-height:44px;border-radius:9px;',
      ...
      '#bs-consent button.accept:hover{background:#228B5B;transform:translateY(-1px)}',
            // #D1D5DB donnait 1,47:1 : un bouton « Refuser » dont on ne voit pas le
      // contour. Sur un bandeau de consentement, refuser doit etre aussi
      // visible qu'accepter — c'est une exigence, pas une preference.
      '#bs-consent button.reject{background:#fff;color:#0d3320;border:1.5px solid #767E8A}',
