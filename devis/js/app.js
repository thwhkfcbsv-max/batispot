...
(function () {
 ...
 // =========================================================
 // DICTÉE VOCALE (Web Speech API Multi-navigateurs iOS/Android)
 // =========================================================
 ...
 if (SR) {
   ...
   let base = "";
   // ⚠️ FUSIONNER, jamais coller. Android redit toute la phrase a chaque
   // resultat marque final : « est-ce que », « est-ce que tu », « est-ce que
   // tu m'entends »… Les concatener produisait exactement ce qu'a vu Moctar
   // le 31/08 : « est-ce queest-ce que tuest-ce que tu m'entends ».
   // Le correctif avait ete pose sur app/devis.html et app-assistant.js mais
   // PAS ici — c'est pourtant cette page qui sert batispot.pro/devis.
   const fusionner = (avant, ajout) => {
     const a = (avant || "").replace(/\s+/g, " ").trim();
     const n = (ajout  || "").replace(/\s+/g, " ").trim();
     if (!a) return n;
     if (!n) return a;
     const la = a.toLowerCase(), ln = n.toLowerCase();
     if (la.endsWith(ln))   return a;   // segment deja pris en compte
     if (ln.startsWith(la)) return n;   // la nouvelle version redit tout, en plus complet
     return a + " " + n;
   };
   recog.onresult = (e) => {
     let interim = "";
     for (let i = e.resultIndex; i < e.results.length; i++) {
       const r = e.results[i];
       // Seul le dernier provisoire compte : les precedents sont deja perimes.
       if (r.isFinal) base = fusionner(base, r[0].transcript);
       else interim = r[0].transcript;
     }
     txt.value = fusionner(base, interim);
     txt.dispatchEvent(new Event("input"));
   };
   recog.onerror = (e) => {
     console.warn("[Voice Error]", e.error);
     micHint.textContent = e.error === "not-allowed" ? "Micro refusé — autorisez l'accès." : "Écoute terminée.";
     stopRec();
   };
   recog.onend = () => {
     if (recording && !isIOS) {
   ...
   micBtn.addEventListener("click", () => {
     recording ? stopRec() : startRec(base = txt.value.trim());
   });
 } else {
   micHint.textContent = "Dictée non disponible — écrivez directement.";
   micBtn.style.opacity = ".4";
 }
 const MIC_SVG = '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2a3 3 0 0 0-3 3v7a3 3 0 0 0 6 0V5a3 3 0 0 0-3-3z"/><path d="M19 10v2a7 7 0 0 1-14 0v-2"/><line x1="12" y1="19" x2="12" y2="22"/></svg>';
 const STOP_SVG = '<svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor"><rect x="6" y="6" width="12" height="12" rx="2.5"/></svg>';
